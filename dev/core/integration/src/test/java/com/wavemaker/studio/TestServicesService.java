...
public class TestServicesService extends StudioTestCase {
    ...
    @Test public void testBasicService() throws Exception {
        ...
        o = invokeService_toObject("servicesService",
                "listServices", null);
    ...
    @Test public void testListDataObjects() throws Exception {
        ...
        Object oWithId = invokeService_toObject("servicesService",
                "listDataObjects", null);
        assertTrue(oWithId instanceof Map<?,?>);
        assertEquals(TestDesignServiceManager.RUNTIME_SERVICE_DOS.length,
                ((Map<?,?>)oWithId).size());
        
        dsm.defineService(sd2);
        oWithId = invokeService_toObject("servicesService",
                "listDataObjects", null);
        assertTrue(oWithId instanceof Map<?,?>);
        Map<?,?> oWithIdMap = (Map<?, ?>) oWithId;
        assertEquals(1 + TestDesignServiceManager.RUNTIME_SERVICE_DOS.length,
                oWithIdMap.size());
        assertTrue(oWithIdMap.containsKey(
                sd2.getLocalTypes().get(0).getTypeName()));
        
        dsm.defineService(sd3);
        oWithId = invokeService_toObject("servicesService",
                "listDataObjects", null);
        assertTrue(oWithId instanceof Map<?,?>);
        assertEquals(3+TestDesignServiceManager.RUNTIME_SERVICE_DOS.length,
                ((Map<?,?>)oWithId).size());
        assertTrue(((Map<?,?>)oWithId).containsKey(
                sd3.getLocalTypes().get(0).getTypeName()));
    }
    
    @Test public void testConvertDataObjects() {
        ...
        Map<String, Map<String, Map<String, Object>>> testee =
            ServicesService.convertDataObjects(daos, services);
        ...
        assertFalse((Boolean)type.get(e1.getName()).get(
                ServicesService.SUPPORTS_RUNTIME_ACCESS_KEY));
        ...
        assertTrue((Boolean)type.get(e2.getName()).get(
                ServicesService.SUPPORTS_RUNTIME_ACCESS_KEY));
    }
