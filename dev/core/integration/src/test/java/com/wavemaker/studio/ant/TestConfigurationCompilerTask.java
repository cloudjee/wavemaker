...
public class TestConfigurationCompilerTask extends StudioTestCase {
	...
	public void testDMCompile() throws Exception {
		...
		File actual = dsm.getServiceDefXml(sd.getServiceId()).getFile();
		...
		assertEquals("smd has been modified!", smd_sd_created,
				smd_sd.lastModified());
		assertEquals("smd2 has been modified!", smd_sd2_created,
				smd_sd2.lastModified());
	...
	public void testBasicWriteConfig() throws Exception {
		...
		Resource resource = new FileResource(dsm.getServiceDefXml(
				sd.getServiceId()).getFile());
		Resource resource2 = new FileResource(dsm.getServiceDefXml(
				sd2.getServiceId()).getFile());
	...
	public void testWriteTypes() throws Exception {
		...
		makeProject(p, false);
		File typesFile = ConfigurationCompiler.getTypesFile(
				pm.getCurrentProject()).getFile();
		File managersFile = ConfigurationCompiler.getRuntimeManagersXml(
				pm.getCurrentProject()).getFile();
	...
	public void testOnlyThroughTask() throws Exception {
		...
		typesFile = ConfigurationCompiler.getTypesFile(pm.getCurrentProject())
				.getFile();
		managersFile = ConfigurationCompiler.getRuntimeManagersXml(
				pm.getCurrentProject()).getFile();
		assertTrue(typesFile.exists());
		assertTrue(managersFile.exists());
		...
		assertFalse(managersFileContents.contains("\"" + sd2.getServiceId()
				+ "\""));
		assertFalse(typesFileContents.contains("\""
				+ sd.getLocalTypes().get(0).getTypeName() + "\""));
		assertFalse(typesFileContents.contains("\""
				+ sd2.getLocalTypes().get(0).getTypeName() + "\""));

		// copy in a service
		File sourceFile = new File(new File(oldProject.getProjectRoot()
				.getFile(), DesignServiceManager.getServicesRelativeDir()),
				sd.getServiceId());
		...
		assertFalse(managersFileContents.contains("\"" + sd2.getServiceId()
				+ "\""));
		assertTrue(typesFileContents.contains("\""
				+ sd.getLocalTypes().get(0).getTypeName() + "\""));
		assertFalse(typesFileContents.contains("\""
				+ sd2.getLocalTypes().get(0).getTypeName() + "\""));

		// copy in another service
		sourceFile = new File(new File(oldProject.getProjectRoot().getFile(),
				DesignServiceManager.getServicesRelativeDir()),
				sd2.getServiceId());
		destServicesFile.mkdir();
		FileUtils.copyDirectoryToDirectory(sourceFile, destServicesFile);
