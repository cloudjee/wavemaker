...
public class TestJavaService extends StudioTestCase {
    ...
    public void testNewService() throws Exception {
        ...
        Object o = invokeService_toObject("javaService", "newClass", new Object[] { serviceId, className });

        File expectedDir = this.dsm.getServiceRuntimeDirectory(serviceId).getFile();
        assertTrue("expectedDir DNE: " + expectedDir, expectedDir.exists());

        File expectedXml = this.dsm.getServiceDefXml(serviceId).getFile();
        ...
        File serviceDefFile = this.dsm.getServiceDefXml(serviceId).getFile();
        assertTrue(serviceDefFile.exists());

        Service service = (Service) this.unmarshaller.unmarshal(serviceDefFile);
    ...
    public void testDuplicateNewService() throws Exception {
        ...
        invokeService_toObject("javaService", "newClass", new Object[] { serviceId, className });
    ...
    public void testNewServicePackage() throws Exception {
        ...
        Object o = invokeService_toObject("javaService", "newClass", new Object[] { serviceId, packageName + "." + className });
        ...
        try {
            this.getClass().getClassLoader().loadClass(packageName + "." + className);
    ...
    public void testSaveOpen() throws Exception {
        ...
        File javaSource = new ClassPathResource("com/wavemaker/tools/project/FooBar.java.txt").getFile();
        assertTrue(javaSource.exists());

        File javaDataSource = new ClassPathResource("com/wavemaker/tools/project/FooBarData.java.txt").getFile();
        File javaDataDest = new File(this.dsm.getServiceRuntimeDirectory(serviceId).getFile(), "foo/bar/baz/data/FooBarData.java");
        FileUtils.copyFile(javaDataSource, javaDataDest);

        File javaData2Source = new ClassPathResource("com/wavemaker/tools/project/FooBarData2.java.txt").getFile();
        File javaData2Dest = new File(this.dsm.getServiceRuntimeDirectory(serviceId).getFile(), "foo/bar/baz/data/FooBarData2.java");
        FileUtils.copyFile(javaData2Source, javaData2Dest);

        o = invokeService_toObject("javaService", "saveClass", new Object[] { serviceId, FileUtils.readFileToString(javaSource) });
        assertNotNull(o);
        assertTrue(o instanceof CompileOutput);
        assertTrue(((CompileOutput) o).isBuildSucceeded());

        service = (Service) this.unmarshaller.unmarshal(serviceDefFile);
        assertEquals(serviceId, service.getId());
        ...
        o = invokeService_toObject("javaService", "openClass", new Object[] { serviceId });
        assertTrue(o instanceof String);
        assertEquals(FileUtils.readFileToString(javaSource), o);

        this.dsm.getProjectManager().closeProject();
    }
    ...
    public void testNewSaveSimple() throws Exception {
        ...
        // copy in the jar
        File javaData2Source = new ClassPathResource("com/wavemaker/common/foojar.jar").getFile();
        File javaData2Dest = new File(this.dsm.getProjectManager().getCurrentProject().getProjectRoot().getFile(), "lib/foojar.jar");
        FileUtils.copyFile(javaData2Source, javaData2Dest, false);

        // give files a chance to update their mtime
        Thread.sleep(2000);

        File javaFile = new File(this.dsm.getServiceRuntimeDirectory(serviceId).getFile(), "foo/bar/FooBar.java");
        ...
        File buildDir = this.dsm.getProjectManager().getCurrentProject().getWebInfClasses().getFile();
        File serviceLibDir = this.dsm.getProjectManager().getCurrentProject().getWebInfLib().getFile();
        ...
        Class<?> serviceClass = ClassLoaderUtils.loadClass("foo.bar.FooBar", cl);
    ...
    public void testJarType() throws Exception {
        ...
        try {
            this.dsm.getClass().getClassLoader().loadClass("foo.bar.FooBar");
    ...
    public void testBadId() throws Exception {
        ...
        try {
            ...
            assertEquals(MessageResource.STUDIO_UNKNOWN_SERVICE.getMessage(serviceName), e.getMessage());
        }
    }
    ...
    public void testBadJava() throws Exception {
        ...
        Object o = invokeService_toObject("javaService", "saveClass", new Object[] {
            serviceId,
            "package foo.bar;\nasdf" + "public class FooBar extends com.wavemaker.runtime.javaservice.JavaServiceSuperClass {\n"
                + "\tpublic FooBar getInstance() { return null; }\n" + "}" });
    ...
    public void testExcludeOtherServicesTypes() throws Exception {
        ...
        invokeService_toObject("javaService", "newClass", new Object[] { oneServiceId, oneClassName });

        // give files a chance to update their mtime
        Thread.sleep(2000);

        invokeService_toObject("javaService", "saveClass", new Object[] {
            oneServiceId,
            "package foo.bar;\n" + "public class FooBar extends com.wavemaker.runtime.javaservice.JavaServiceSuperClass {\n"
                + "\tpublic FooBar getInstance() { return null; }\n" + "}" });

        invokeService_toObject("javaService", "newClass", new Object[] { twoServiceId, twoClassName });
        ...
        File expectedDir = this.dsm.getServiceRuntimeDirectory(twoServiceId).getFile();
        assertTrue("expectedDir DNE: " + expectedDir, expectedDir.exists());

        File serviceDefFile = this.dsm.getServiceDefXml(twoServiceId).getFile();
        assertTrue(serviceDefFile.exists());
        ...
        expectedDir = this.dsm.getServiceRuntimeDirectory(oneServiceId).getFile();
        assertTrue("expectedDir DNE: " + expectedDir, expectedDir.exists());

        serviceDefFile = this.dsm.getServiceDefXml(oneServiceId).getFile();
        assertTrue(serviceDefFile.exists());
        ...
        assertEquals(1, service.getDataobjects().getDataobject().size());
        assertEquals(oneClassName, service.getDataobjects().getDataobject().get(0).getJavaType());
    }
    ...
    public void XXXtestGreedyJavaService_LinkedType() throws Exception {
        ...
        // create Bar
        invokeService_toObject("javaService", "newClass", new Object[] { barServiceId, "Bar" });
        Thread.sleep(2000);
        ...
        for (DataObject dao : this.dsm.getDataObjects()) {
            assertFalse(dao.getJavaType().equals("Bar"));
        }

        // add Foo service
        invokeService_toObject("javaService", "newClass", new Object[] { fooServiceId, "Foo" });
        Thread.sleep(2000);
