...
public class TestJavaService extends StudioTestCase {
	...
	public void testNewService() throws Exception {
		...
		File expectedDir = dsm.getServiceRuntimeDirectory(serviceId).getFile();
		assertTrue("expectedDir DNE: " + expectedDir, expectedDir.exists());

		File expectedXml = dsm.getServiceDefXml(serviceId).getFile();
		...
		File serviceDefFile = dsm.getServiceDefXml(serviceId).getFile();
	...
	public void testSaveOpen() throws Exception {
		...
		File javaDataDest = new File(dsm.getServiceRuntimeDirectory(serviceId)
				.getFile(), "foo/bar/baz/data/FooBarData.java");
		...
		File javaData2Dest = new File(dsm.getServiceRuntimeDirectory(serviceId)
				.getFile(), "foo/bar/baz/data/FooBarData2.java");
		FileUtils.copyFile(javaData2Source, javaData2Dest);

		o = invokeService_toObject("javaService", "saveClass", new Object[] {
				serviceId, FileUtils.readFileToString(javaSource) });
		assertNotNull(o);
	...
	public void testNewSaveSimple() throws Exception {
		...
		File javaData2Dest = new File(dsm.getProjectManager()
				.getCurrentProject().getProjectRoot().getFile(),
				"lib/foojar.jar");
		...
		File buildDir = dsm.getProjectManager().getCurrentProject()
				.getWebInfClasses().getFile();
	...
	public void testBadId() throws Exception {
		...
		try {
			...
			assertEquals(
					MessageResource.STUDIO_UNKNOWN_SERVICE
							.getMessage(serviceName),
					e.getMessage());
		}
	}
	...
	public void testExcludeOtherServicesTypes() throws Exception {
		...
		File serviceDefFile = dsm.getServiceDefXml(twoServiceId).getFile();
		assertTrue(serviceDefFile.exists());
		...
		expectedDir = dsm.getServiceRuntimeDirectory(oneServiceId).getFile();
		assertTrue("expectedDir DNE: " + expectedDir, expectedDir.exists());

		serviceDefFile = dsm.getServiceDefXml(oneServiceId).getFile();
		assertTrue(serviceDefFile.exists());
