...
public class TestConfigurationCompiler extends StudioTestCase {
	...
	public void setUp() throws Exception {
		...
		fileService = new AbstractFileService(new LocalStudioConfiguration()) {
	...
	public void testGenerateManagers() throws Exception {
		...
		Project project = new Project(new FileSystemResource(actualFile
				.getParentFile().getAbsolutePath() + "/"),
				new LocalStudioConfiguration());

		SortedSet<Service> services = new TreeSet<Service>(
		...
		ConfigurationCompiler.generateManagers(project, new FileSystemResource(
				actualFile), services);
		...
		Beans actual = SpringConfigSupport.readBeans(new FileSystemResource(
				actualFile.getAbsolutePath() + "/"), fileService);
		Beans expected = SpringConfigSupport.readBeans(new FileSystemResource(
				expectedFile.getAbsolutePath() + "/"), fileService);
	...
	public void testGenerateServicesWithoutSpringFile() throws Exception {
		...
		try {
			ConfigurationCompiler.generateServices(project,
					new FileSystemResource(actualFile), services);
			fail("no exception");
		} catch (WMRuntimeException e) {
			assertEquals(e.getMessageId(),
					MessageResource.NO_EXTERNAL_BEAN_DEF.getId());
		}
	}
	...
	public void testGenerateSMDs() throws Exception {
		...
		ConfigurationCompiler.generateSMDs(pm.getCurrentProject(), services);

		File serviceA_SMD = ConfigurationCompiler.getSmdFile(
				pm.getCurrentProject(), a.getId()).getFile();
		File serviceB_SMD = ConfigurationCompiler.getSmdFile(
				pm.getCurrentProject(), b.getId()).getFile();
		assertTrue(serviceA_SMD.exists());
		assertTrue(serviceB_SMD.exists());

		assertEquals(StringUtils.deleteWhitespace(FileUtils
				.readFileToString(expectedAFile)),
				StringUtils.deleteWhitespace(FileUtils
						.readFileToString(serviceA_SMD)));

		JSONObject jsonA = (JSONObject) JSONUnmarshaller.unmarshal(FileUtils
				.readFileToString(serviceA_SMD));
		JSONObject expectedA = (JSONObject) JSONUnmarshaller
				.unmarshal(FileUtils.readFileToString(expectedAFile));
		assertEquals(expectedA, jsonA);

		JSONObject jsonB = (JSONObject) JSONUnmarshaller.unmarshal(FileUtils
				.readFileToString(serviceB_SMD));
		JSONObject expectedB = (JSONObject) JSONUnmarshaller
				.unmarshal(FileUtils.readFileToString(expectedBFile));
	...
	public void testGetMethod_NoReturn() throws Exception {
		...
		SortedSet<Method> methods = ConfigurationCompiler
				.getMethods(ops, "foo");
	...
	@Test
	public void testGetMethod_GoodMethodOverloadNoArgsReorder()
			throws Exception {
	...
	public void testBadServiceDefXml() throws Exception {
		...
		String badServiceData = FileUtils
				.readFileToString((new ClassPathResource(
						"com/wavemaker/tools/service/bad-servicedef.xml"))
						.getFile());

		FileUtils.writeStringToFile(dsm.getServiceDefXml("sakila").getFile(),
				badServiceData);
	...
	public void testEnclosedClassRegularName() throws Exception {
		...
		String serviceDef = FileUtils.readFileToString((new ClassPathResource(
				"com/wavemaker/tools/service/enclosedclass-servicedef.xml"))
				.getFile());
		FileUtils.writeStringToFile(dsm.getServiceDefXml("asdf").getFile(),
				serviceDef);
		...
		String typesString = FileUtils.readFileToString(ConfigurationCompiler
				.getTypesFile(pm.getCurrentProject()).getFile());
	...
	public void testGetTypesFromServices() throws Exception {
		...
		Types types = ConfigurationCompiler
				.getTypesFromServices(services, null);
		assertNotNull(types);
	...
	public void testGetTypesFromDataObjects() throws Exception {
		...
		ConfigurationCompiler
				.getTypesFromDataObjects("fooService", daos, types);
	...
	protected static void assertBeansEquals(Beans expected, Beans actual)
		...
		Writer writer = new OutputStreamWriter(new FileOutputStream(
				actualFileOut), ServerConstants.DEFAULT_ENCODING);
