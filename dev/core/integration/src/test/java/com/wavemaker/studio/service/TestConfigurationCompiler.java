...
public class TestConfigurationCompiler extends StudioTestCase {
    ...
    public void setUp() throws Exception {
        ...
        dsm = (DesignServiceManager) getApplicationContext().getBean("designServiceManager");
    ...
    @Test public void testGenerateManagers() throws Exception {
        
        File actualFile = File.createTempFile(
                "TestRuntimeConfigurationGeneration", ".tmp");
        actualFile.deleteOnExit();

        Project project = new Project(actualFile.getParentFile());
        ...
        ConfigurationCompiler.generateManagers(project, actualFile, services);
        
        File expectedFile = (new ClassPathResource(
                "com/wavemaker/tools/service/generate-managers-test.xml")).
                getFile();
        
        Beans actual = SpringConfigSupport.readBeans(actualFile, fileService);
        Beans expected = SpringConfigSupport.readBeans(expectedFile, fileService);
        
        assertBeansEquals(expected, actual);
    }
    
    @Test public void testGenerateServicesWithoutSpringFile() throws Exception {
        ...
        try {
            ConfigurationCompiler.generateServices(project, actualFile,
                    services);
            fail("no exception");
        } catch (WMRuntimeException e) {
            assertEquals(e.getMessageId(), Resource.NO_EXTERNAL_BEAN_DEF.getId());
        }
    }

    @Test public void testGenerateSMDs() throws Exception {
        
        File expectedAFile = (new ClassPathResource(
                "com/wavemaker/tools/service/testgeneratesmds_a.smd")).
                getFile();
        File expectedBFile = (new ClassPathResource(
                "com/wavemaker/tools/service/testgeneratesmds_b.smd")).
                getFile();
        ...
        ConfigurationCompiler.generateSMDs(pm.getCurrentProject(),
                services);
        
        File serviceA_SMD = ConfigurationCompiler.getSmdFile(
                pm.getCurrentProject(), a.getId());
        File serviceB_SMD = ConfigurationCompiler.getSmdFile(
                pm.getCurrentProject(), b.getId());
        ...
        JSONObject jsonA = (JSONObject) JSONUnmarshaller.unmarshal(
                FileUtils.readFileToString(serviceA_SMD));
        JSONObject expectedA = (JSONObject) JSONUnmarshaller.unmarshal(
                FileUtils.readFileToString(expectedAFile));
        assertEquals(expectedA, jsonA);
        
        JSONObject jsonB = (JSONObject) JSONUnmarshaller.unmarshal(
                FileUtils.readFileToString(serviceB_SMD));
        JSONObject expectedB = (JSONObject) JSONUnmarshaller.unmarshal(
                FileUtils.readFileToString(expectedBFile));
        assertEquals(expectedB, jsonB);
    }

    @Test public void testGetMethod_NoReturn() throws Exception {
    ...
    @Test public void testGetMethod_GoodMethodOverload() throws Exception {
    ...
    @Test public void testGetMethod_GoodMethodOverloadNoArgs() throws Exception {
    ...
    @Test public void testGetMethod_GoodMethodOverloadNoArgsReorder() throws Exception {
    ...
    @Test public void testGetMethod_BadMethodOverload() throws Exception {
    ...
    @Test public void testBadServiceDefXml() throws Exception {
        
        makeProject("testBadServiceDefXml");
        
        String badServiceData = FileUtils.readFileToString(
                (new ClassPathResource(
                        "com/wavemaker/tools/service/bad-servicedef.xml")).
                        getFile());
        
        FileUtils.writeStringToFile(dsm.getServiceDefXml("sakila"),
                badServiceData);
    ...
    @Test public void testEnclosedClassRegularName() throws Exception {
        
        makeProject("testEnclosedClassRegularName");
        
        String serviceDef = FileUtils.readFileToString(
                (new ClassPathResource(
                        "com/wavemaker/tools/service/enclosedclass-servicedef.xml")).
                        getFile());
        FileUtils.writeStringToFile(dsm.getServiceDefXml("asdf"),
                serviceDef);
        
        // make sure we're getting the types properly
        Types types = ConfigurationCompiler.getTypesFromServices(
                dsm.getServices(), dsm.getPrimitiveDataObjects());
        ...
        ConfigurationCompiler.generateTypes(pm.getCurrentProject(),
                ConfigurationCompiler.getTypesFile(pm.getCurrentProject()),
                dsm.getServices(), dsm.getPrimitiveDataObjects());
        
        String typesString = FileUtils.readFileToString(
                ConfigurationCompiler.getTypesFile(pm.getCurrentProject()));
        typesString = typesString.substring(
                ConfigurationCompiler.WM_TYPES_PREPEND.length(),
                typesString.length()-ConfigurationCompiler.WM_TYPES_APPEND.length());
    ...
    @Test public void testGetTypesFromServices() throws Exception {

        SortedSet<Service> services = getTestServices();
        
        Types types = ConfigurationCompiler.getTypesFromServices(services,
                null);
        assertNotNull(types);
    ...
    @Test public void testGetTypesFromDataObjects() throws Exception {
        ...
        ConfigurationCompiler.getTypesFromDataObjects("primitive",
                dsm.getPrimitiveDataObjects(), types);
    ...
    protected static void assertBeansEquals(Beans expected, Beans actual)
            throws IOException, JAXBException {
        
        File actualFileOut = File.createTempFile(
                "TestRuntimeConfigurationGeneration", ".tmp");
        actualFileOut.deleteOnExit();
        File expectedFileOut = File.createTempFile(
                "TestRuntimeConfigurationGeneration", ".tmp");
        expectedFileOut.deleteOnExit();
        
        Writer writer = new OutputStreamWriter(new FileOutputStream(actualFileOut),
                ServerConstants.DEFAULT_ENCODING);
        SpringConfigSupport.writeBeans(actual, writer);
        writer.close();
        
        writer = new OutputStreamWriter(new FileOutputStream(expectedFileOut),
                    ServerConstants.DEFAULT_ENCODING);
        SpringConfigSupport.writeBeans(expected, writer);
        writer.close();
        
        assertEquals(FileUtils.readFileToString(expectedFileOut),
                FileUtils.readFileToString(actualFileOut));
    }
