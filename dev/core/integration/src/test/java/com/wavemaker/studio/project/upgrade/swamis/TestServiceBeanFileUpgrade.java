...
public class TestServiceBeanFileUpgrade extends StudioTestCase {
	...
	public void testUpgrade() throws Exception {
		...
		assertEquals(
				"verbose was: " + upgradeInfo.getVerbose() + ", messages: "
		...
		File designTimeXml = new File(dsm.getServiceDesigntimeDirectory(
				serviceId).getFile(),
	...
	public void testUpgradeTwoProjects() throws Exception {
		...
		try {
			...
			assertEquals(MessageResource.NO_EXTERNAL_BEAN_DEF.getId(),
					e.getMessageId());
		}
		...
		File designTimeXml2 = new File(dsm.getServiceDesigntimeDirectory(
				serviceId2).getFile(),
				DesignServiceManager.getServiceXmlRelative());
		Service service2Prime = (Service) unmarshaller
				.unmarshal(designTimeXml2);
	...
	private void createServiceStub(DesignServiceManager dsm, String serviceId,
			String klass) throws JAXBException, IOException {
