...
public class TestServiceBeanFileUpgrade extends StudioTestCase {
    ...
    public TestServiceBeanFileUpgrade() throws JAXBException {
        super();
        this.definitionsContext = JAXBContext.newInstance("com.wavemaker.tools.service.definitions");
    }
    ...
    public void testUpgrade() throws Exception {
        ...
        // manually create a second, so this one won't have a spring bean def
        // file
        String serviceId = "otherService";
        String serviceClass = "foo.bar.OtherService";
        createServiceStub(dsm, serviceId, serviceClass);

        File serviceBean = dsm.getServiceBeanXml(serviceId).getFile();
        ...
        assertEquals("verbose was: " + upgradeInfo.getVerbose() + ", messages: " + upgradeInfo.getMessages(),
            "Converted bean definitions to a separate file for services: otherService", upgradeInfo.getVerbose().get("-1.0").get(0));

        assertTrue(serviceBean.exists());
        assertTrue(FileUtils.readFileToString(serviceBean).contains("\"" + serviceId + "\""));

        Unmarshaller unmarshaller = this.definitionsContext.createUnmarshaller();
        File designTimeXml = new File(dsm.getServiceDesigntimeDirectory(serviceId).getFile(), DesignServiceManager.getServiceXmlRelative());
    ...
    // MAV-1813
    @Test
    public void testUpgradeTwoProjects() throws Exception {
        ...
        File serviceBean2 = dsm.getServiceBeanXml(serviceId2).getFile();
        ...
        try {
            ...
            assertEquals(MessageResource.NO_EXTERNAL_BEAN_DEF.getId(), e.getMessageId());
        }
        ...
        ut.doUpgrade(project, upgradeInfo);
        assertEquals("verbose was: " + upgradeInfo.getVerbose() + ", messages: " + upgradeInfo.getMessages(),
            "Converted bean definitions to a separate file for services: service, service2", upgradeInfo.getVerbose().get("-1.0").get(0));
        ...
        assertTrue(FileUtils.readFileToString(serviceBean2).contains("\"" + serviceId2 + "\""));

        File designTimeXml2 = new File(dsm.getServiceDesigntimeDirectory(serviceId2).getFile(), DesignServiceManager.getServiceXmlRelative());
    ...
    private void createServiceStub(DesignServiceManager dsm, String serviceId, String klass) throws JAXBException, IOException {
        ...
        dsm.getServiceHome(serviceId).getFile().mkdir();
        dsm.getServiceDesigntimeDirectory(serviceId).getFile().mkdir();
        File designTimeXml = new File(dsm.getServiceDesigntimeDirectory(serviceId).getFile(), DesignServiceManager.getServiceXmlRelative());
        assertFalse(designTimeXml.exists());
        assertTrue(designTimeXml.getParentFile().exists());

        Marshaller marshaller = this.definitionsContext.createMarshaller();
