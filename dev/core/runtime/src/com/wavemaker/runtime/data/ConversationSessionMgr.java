 ...
 * 
 * The bind/unbind calls are only for getting SessionFactory.getCurrentSession
 * to work. Since we keep track of the session and never access it through those
 * calls, we don't really need to call bind/unbind. However, if we expose the
 * Hibernate API to our users, we better make sure getCurrentSession works as
 * expected.
 * 
 * @author Simon Toens
 */
