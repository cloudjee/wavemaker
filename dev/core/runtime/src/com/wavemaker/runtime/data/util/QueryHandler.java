...
public class QueryHandler implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method m, Object[] args) throws Throwable {
        ...
        if (!methodName.equals("save") &&
                ...
                !methodName.equals("getNamedQuery") &&
        ...
        if (methodName.equalsIgnoreCase("createQuery")) {
            ...
            Query qry = (Query)m.invoke(target, qryStr);
            if (tenantAdded) qry.setInteger("wmtidval", tid);
            return qry;
        } else if (methodName.equalsIgnoreCase("createSQLQuery")) {
            qryStr = modifySQL(args[0], tFldName, tid);
            SQLQuery sqlqry = (SQLQuery)m.invoke(target, qryStr);
            if (tenantAdded) sqlqry.setInteger("wmtidval", tid);
            return sqlqry;
            ...
            qryStr = qd.getQuery();
            Query qry = (Query)m.invoke(target, args);
            if (qryStr.contains("wmtidval")) qry.setInteger("wmtidval", tid);
            return qry;
            ...
            if (tidExists) setter.invoke(o, tid);
    ...
    public String modifySQL (Object o, String fieldName, int tid) {
        ...
        for (int i=0; i<len; i++) {
            ...
            if (byPassChar(word)) {
                ...
                if (appendSpace(sb.toString(), word)) sb.append(" ");
                sb.append(word);
                if (qid > -1) {
                    tm.put(qid, sq);
                }
            ...
            } else if (word.equalsIgnoreCase("insert") && words.get(i+1).equalsIgnoreCase("into") &&
                    i == 0) {
                if (appendSpace(sb.toString(), word)) sb.append(" ");
                sb.append(word);
                qid++;
                ...
                if (tidInsertPosition >= openingInsertPosition) tidInsertPosition++;
                sq.inWhere = true;
                sq.inFrom = false;
                sq.inUpdate = false;
                sq.inSet = false;
                ...
                if (appendSpace(sb.toString(), word)) sb.append(" ");
                sb.append(word);
            } else if (word.equalsIgnoreCase("inner") || //misc key words that must turn off inForm flag
                    word.equalsIgnoreCase("left") ||
                    word.equalsIgnoreCase("right") ||
                    word.equalsIgnoreCase("full") ||
            ...
            } else if (sq.queryType.equalsIgnoreCase("select") && sq.inFrom ||
                        sq.queryType.equalsIgnoreCase("update") && sq.inUpdate ||
        ...
        if (sq.inFrom && !sq.qryIncludeAlias) {
            ...
            if (appendSpace(sb.toString(), aliasName)) sb.append(" ");
            sb.append(aliasName);
    ...
    private boolean appendSpace(String str, String word) {
        if (str.length() == 0 || str.endsWith("(") || str.endsWith(" ")) return false;
       ...
       private SingleQuery (String qtype, int qid, boolean flag) {
        ...
        private StringBuffer appendTableAlias (StringBuffer sb, List<String> words, int indx, String fieldName) {
            if ((this.queryType.equalsIgnoreCase("select") && this.inFrom ||
                 ...
                 this.queryType.equalsIgnoreCase("delete") && this.inDelete) &&
