...
public class TestConfigurationStore extends WMTestCase {
    ...
    public void testNonVersioned() throws Exception {
        ...
        assertEquals("bar", ConfigurationStore.getPreference(getClass(),
                    "foo", "baz"));
        ConfigurationStore.removePreference(getClass(), "foo");
        assertEquals("baz", ConfigurationStore.getPreference(getClass(),
                    "foo", "baz"));
    }

    public void testGetRootVersionedNode() throws Exception {

        Preferences p = ConfigurationStore.getRootVersionedNode(getClass(),
                "foo");
        assertEquals("/com/wavemaker/tools/config/wm_versioned__foo",
                p.absolutePath());
    }
    ...
    public void testGetLessThanEqualVersionedNode() throws Exception {
        ...
        Preferences pp = ConfigurationStore.getLessThanEqualVersionedNode(
                getClass(), key, vi);
        assertEquals(expected, pp.absolutePath());

        // try a lower version, so this should return the default location
        pp = ConfigurationStore.getLessThanEqualVersionedNode(getClass(), key,
                new VersionInfo("1.2.0"));
        assertEquals(Preferences.userNodeForPackage(getClass()).absolutePath(),
                pp.absolutePath());

        // and add a bad preferences string
        Preferences pbad = ConfigurationStore.getRootVersionedNode(getClass(),
                key);
        ...
        Preferences phigher = ConfigurationStore.getRootVersionedNode(
                getClass(), key);
    ...
    public void testGetExactVersionedNode() throws Exception {
        ...
        Preferences pp = ConfigurationStore.getExactVersionedNode(getClass(),
                key, vi);
        assertEquals(expected, pp.absolutePath());
    ...
    public void testVersioned() throws Exception {
        ...
        assertEquals("nonversioned", ConfigurationStore.getPreference(
                    getClass(), key, null));

        ConfigurationStore.setVersionedPreference(getClass(), key, "versioned");
        assertEquals("versioned", ConfigurationStore.getPreference(getClass(),
                    key, null));

        ConfigurationStore.setVersionedPreference(getClass(), key, "versioned2");
        assertEquals("versioned2", ConfigurationStore.getPreference(getClass(),
                    key, null));
    }
    ...
    public void testSetVersioned_NoPreexisting() throws Exception {
        ...
        Preferences p = ConfigurationStore.getLessThanEqualVersionedNode(
                getClass(), key, new VersionInfo("1.1.1"));
        ...
        p = ConfigurationStore.getLessThanEqualVersionedNode(
                getClass(), key, new VersionInfo("1.1.1"));
