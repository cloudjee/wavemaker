...
/**
 * Main configuration storage interface for WaveMaker.  Wraps
 * {@link Preferences} in a system-independent manner.
 *
 * Also adds versioning to preferences.  When a preference is marked as
 ...
 * <ol>
 * <li>If only the requested key exists (but it has no children), the value
 * from that key is returned.</li>
 * <li>If the requested key has children which have proper version information,
 * the version key less than or equal to the current version will be
 * returned.  If all explicit versions are greater than the current version,
 * the non-versioned key will be used.</li>
 * </ol>
 *
 * When removing versioned Preferences, all versions will be removed at the
...
public class ConfigurationStore {
    ...
    /** Get the preference as a String, or if that doesn't exist, use the
     * default.  If the preference has been versioned, this will return the
     * appropriate versioned preference.
     */
    ...
    /** Get the preference as a boolean, or if that doesn't exist, use the
     * default.  If the preference has been versioned, this will return the
     * appropriate versioned preference.
     */
    public static boolean getPreferenceBoolean(Class<?> klass, String key, boolean defaultValue) {
        return getLessThanEqualVersionedNode(klass, key).getBoolean(key,
                defaultValue);
    }
    
    /** Get the preference as an int, or if that doesn't exist, use the
     * default.  If the preference has been versioned, this will return the
     * appropriate versioned preference.
     */
    public static int getPreferenceInt(Class<?> klass, String key, int defaultValue) {
        return getLessThanEqualVersionedNode(klass, key).getInt(key,
                defaultValue);
    }
    ...
    protected static Preferences getRootVersionedNode(Class<?> klass,
            String key) {
    ...
    protected static Preferences getLessThanEqualVersionedNode(Class<?> klass,
            String key) {
    ...
    protected static Preferences getLessThanEqualVersionedNode(Class<?> klass,
            String key, VersionInfo version) {
        ...
        try {
            ...
            for (String name: p.childrenNames()) {
                ...
                if (vi.compareTo(currentStudioVersion)<=0 &&
                        ((null==currentPrefVersion) ||
                         (currentPrefVersion.compareTo(vi)<0))) {
    ...
    protected static Preferences getExactVersionedNode(Class<?> klass,
            String key) {
