...
public class Response
{
     ...
     * as defined by Constants.X_STORAGE_TOKEN and if this exists it returns its value otherwise the value returned will be null.
     *
     * @return null if the user is not logged into Cloud FS or the Storage token
     */
    public String getAuthToken ()
    {
        return getResponseHeader(CommonUtil.getProperty("auth_token_name", Constants.X_AUTH_TOKEN)).getValue();
    }

    /**
     * This method makes no assumptions about the user having been logged in.  It simply looks for the Storage URL header
     * as defined by Constants.X_STORAGE_URL and if this exists it returns its value otherwise the value returned will be null.
     *
     * @return null if the user is not logged into Cloud FS or the Storage URL
     */
    public String getServerManagementURL ()
    {
       return getResponseHeader(Constants.X_SERVER_MANAGEMENT_URL).getValue();
    }
    ...
    {
       return getResponseHeader(Constants.X_STORAGE_URL).getValue();
    }

    /**
     * This method makes no assumptions about the user having been logged in.  It simply looks for the CDN Management URL header
     * as defined by Constants.X_CDN_MANAGEMENT_URL and if this exists it returns its value otherwise the value returned will be null.
     *
     * @return null if the user is not logged into Cloud FS or the Storage URL
     */
    public String getCDNManagementURL ()
    {
    	Header header = getResponseHeader(Constants.X_CDN_MANAGEMENT_URL);
    ...
    {
    	Header hdr = getResponseHeader(Constants.E_TAG);
    ...
    {
        Header contCountHeader = getResponseHeader (Constants.X_CONTAINER_OBJECT_COUNT);
    ...
    {
        Header contBytesUsedHeader = getResponseHeader (Constants.X_CONTAINER_BYTES_USED);
    ...
    {
        Header contCountHeader = getResponseHeader (Constants.X_ACCOUNT_CONTAINER_COUNT);
        if (contCountHeader != null )
          return Integer.parseInt(contCountHeader.getValue());
        return -1;
    }
    ...
    {
        Header accountBytesUsedHeader = getResponseHeader (Constants.X_ACCOUNT_BYTES_USED);
    ...
    {
        Header cdnHeader = getResponseHeader (Constants.X_CDN_URI);
        if (cdnHeader != null )
