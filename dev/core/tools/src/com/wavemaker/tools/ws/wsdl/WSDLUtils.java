...
public class WSDLUtils {
     ...
     */
    public static Definition readDefinition(String wsdlURI)
            throws WSDLException {
        try {
     ...
     *            The WSDL definition.
     * @param wsdlFile
     *            The file that this definition is going to write to.
     * @throws WSDLException
     */
    ...
    /**
     * Returns a service name for the WSDL. First check the service
     * element, and if it is not defined, then use portType element.
     * 
     * @param definition
     *            WSDL definition
    ...
    private static <T extends ExtensibilityElement> T getExtensiblityElement(
            List<?> list, Class<T> clazz) {
        List<T> elements = getExtensiblityElements(list, clazz);
        return elements.isEmpty() ? null : elements.get(0);
    }

    private static <T extends ExtensibilityElement> List<T> getExtensiblityElements(
            List<?> list, Class<T> clazz) {
    ...
    public static String getStyle(Binding binding) {
        SOAPBinding soapBinding = (SOAPBinding) getExtensiblityElement(binding
                .getExtensibilityElements(), SOAPBinding.class);
        if (soapBinding != null) {
            return soapBinding.getStyle();
        }

        SOAP12Binding soap12Binding = (SOAP12Binding) getExtensiblityElement(
                binding.getExtensibilityElements(), SOAP12Binding.class);
    ...
    public static boolean isInputSOAPEncoded(BindingOperation bindingOperation) {
        SOAPBody soapBody = (SOAPBody) getExtensiblityElement(bindingOperation
        ...
        if (soapBody != null) {
            return soapBody.getUse() != null
                    ...
                    && (soapBody.getEncodingStyles() == null || soapBody
                            .getEncodingStyles().contains(
                                    Constants.SOAP_ENCODING_NS));
        }

        SOAP12Body soap12Body = (SOAP12Body) getExtensiblityElement(
        ...
        if (soap12Body != null) {
            return soap12Body.getUse() != null
                    ...
                    && (soap12Body.getEncodingStyle() == null || soap12Body
                            .getEncodingStyle().equals(
                                    Constants.SOAP_ENCODING_NS));
        }
    ...
    public static boolean isSOAPEncoded(Definition definition) {
        ...
        if (binding != null) {
            List<BindingOperation> bindingOperations = CastUtils.cast(binding
                    .getBindingOperations(), BindingOperation.class);
    ...
    public static Binding getBindingForSOAP(Definition definition, Operation operation) {
        ...
        while (names.hasNext()) {
            ...
            if (operation == null || 
                    contains(CastUtils.cast(portType.getOperations(),
                            Operation.class), operation)) {
                SOAPBinding soapBinding = (SOAPBinding) getExtensiblityElement(
    ...
    private static boolean contains(List<Operation> operationList,
            Operation operation) {
    ...
    public static Map<String, Element> getSchemaMap(Types types) {
        ...
        if (types != null) {
            for (Iterator<Object> iterator = CastUtils.cast(
                    types.getExtensibilityElements().iterator()); iterator.hasNext();) {
                extensibilityElement = iterator.next();

                if (extensibilityElement instanceof Schema) {
                    Element schemaElement = ((Schema) extensibilityElement)
                            .getElement();
                    schemaMap.put(schemaElement
                            .getAttribute(Constants.TARGET_NAMESPACE),
                            schemaElement);
                }
            }
        }
    ...
    private static List<SoapHeader> getSoapHeaders(List<?> list) {
        ...
        List<SOAPHeader> soapHeaders = getExtensiblityElements(list,
                SOAPHeader.class);
        if (soapHeaders != null && !soapHeaders.isEmpty()) {
            for (SOAPHeader header : soapHeaders)
                result.add(new Soap11Header(header));
        } else {
            List<SOAP12Header> soap12Headers = getExtensiblityElements(list,
                    SOAP12Header.class);
    ...
    public static List<SoapHeader> getInputSoapHeaders(Definition definition,
        ...
        return getSoapHeaders(bindingOperation.getBindingInput()
                .getExtensibilityElements());
    }

    private static List<Part> getFaultParts(BindingOperation bindingOperation,
            String faultName) {
        List<Part> result = new ArrayList<Part>();

        BindingFault bindingFault = bindingOperation.getBindingFault(faultName);
        SOAPFault soapFault = (SOAPFault) getExtensiblityElement(bindingFault
                .getExtensibilityElements(), SOAPFault.class);

        if (soapFault != null && soapFault.getName() != null) {
            List<Part> plist = CastUtils.cast(bindingOperation.getOperation()
                    .getFault(soapFault.getName()).getMessage()
                    .getOrderedParts(null));
            result.addAll(plist);
        } else {
            SOAP12Fault soap12Fault = (SOAP12Fault) getExtensiblityElement(
                    bindingFault.getExtensibilityElements(), SOAP12Fault.class);

            if (soap12Fault != null && soap12Fault.getName() != null) {
                List<Part> plist = CastUtils.cast(bindingOperation
                        .getOperation().getFault(soap12Fault.getName())
                        .getMessage().getOrderedParts(null));
                result.addAll(plist);
            } else {
                List<Part> plist = CastUtils.cast(bindingOperation
                        .getOperation().getFault(faultName).getMessage()
                        .getOrderedParts(null));
                result.addAll(plist);
            }
        }
    ...
    public static List<Part> getFaults(Definition definition,
        ...
        Iterator<String> iter = CastUtils.cast(bindingOperation
                .getBindingFaults().keySet().iterator());
    ...
    public static SoapAddress getSoapAddress(Port port) {
        SOAPAddress soapAddress = getExtensiblityElement(port
                .getExtensibilityElements(), SOAPAddress.class);
        if (soapAddress != null) {
            return new Soap11Address(soapAddress);
        }
        SOAP12Address soap12Address = getExtensiblityElement(port
                .getExtensibilityElements(), SOAP12Address.class);
