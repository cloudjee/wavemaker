...
public class UpgradeManager implements InitializingBean {
    ...
    public UpgradeInfo doUpgrades(Project project) {
        ...
        if (projectVersion > projectMaxVersion) {
            throw new WMRuntimeException(Resource.PROJECT_NEWER_THAN_STUDIO,
            ...
            File exportFile = new File(new File(project.getProjectRoot(),
                    DeploymentManager.EXPORT_DIR_DEFAULT), project
                    .getProjectName()
                    + "-upgrade-" + projectVersion + ".zip");
            deploymentManager.exportProject(exportFile.getAbsolutePath());
            ...
            for (Entry<Double, List<UpgradeTask>> entry: getUpgrades().entrySet()) {
                ...
                for (UpgradeTask task: entry.getValue()) {
                    logger.debug("Running upgrade task "+task+" on project "+
                            project);
    ...
    public void doStudioUpgrade() {
        ...
        if (studioVersion > studioMaxVersion) {
            ...
            for (Entry<Double, List<StudioUpgradeTask>> entry:
                    getStudioUpgrades().entrySet()) {
