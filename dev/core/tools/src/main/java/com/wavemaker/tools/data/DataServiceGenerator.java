...
public class DataServiceGenerator extends ServiceGenerator {
	...
	public DataServiceGenerator(GenerationConfiguration configuration,
		...
		this.constantsClass = new BeanGenerator(
				DataServiceUtils.getConstantsClassName(serviceClass));
	...
	protected void postGenerateClassBody(JDefinedClass cls) {
		...
		if (generateMain) {
			...
			JVar v = b.decl(
					...
					JExpr.cast(
							t,
							codeModel.ref(SpringUtils.class)
									.staticInvoke(GET_BEAN_METHOD_NAME)
									.arg(springConfig).arg(beanName)));
	...
	private void addRead(JDefinedClass cls) {
		JMethod method = cls.method(JMod.PUBLIC, TypedServiceReturn.class,
				READ_CRUD_OP);
		...
		JVar pagingOptionsParam = method.param(PagingOptions.class,
				"pagingOptions");
		JInvocation exp = dataServiceVar.invoke(INVOKE_METHOD_NAME);
		exp.arg(taskMgrVar.invoke(DefaultTaskManager.GET_READ_TASK))
				.arg(rootTypeParam).arg(instanceParam)
				.arg(propertyOptionsParam).arg(pagingOptionsParam);
		method.body()._return(
	...
	private void delegate(JDefinedClass cls, String name, Class<?>[] params,
		...
		if (useNDCLogging) {
			tryBlock._finally().block()
					.staticInvoke(codeModel.ref(NDC.class), NDC_POP);
		}
	}
	...
	protected void writeConstantsClass() {
		...
		try {
			File f = new File(configuration.getOutputDirectory().getFile(),
					relPath);
