...
public class FlowSupport {
	...
	private final String xsdHeader = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
			+ "<xs:schema attributeFormDefault=\"unqualified\" elementFormDefault=\"qualified\" xmlns:xs=\"http://www.w3.org/2001/XMLSchema\">\n";
	private final String xsdBody = "  <xs:element name=\"operation_name_result\" type=\"operation_name_ResultType\"/>\n"
			...
			+ "      record_content_\n"
			+ "    </xs:sequence>\n" + "  </xs:complexType>";
	private final String xsdTrailer = "</xs:schema>";
	...
	public String listProjects(String host, String port, String sessionId)
			throws Exception {
		String result;
		...
		String endPointAddress = "http://" + host + ":" + port
				+ "/flow-rest/project/list.json";
		String postData = "sessionid=" + sessionId;
		QName thisQName = new QName(endPointAddress, "asteriaServer");

		result = HTTPBindingSupport.getResponseObject(thisQName, thisQName,
				endPointAddress, HTTPBindingSupport.HTTPRequestMethod.POST,
	...
	public String listFlows(String host, String port, String projectName,
			String sessionId) throws Exception {
		return getFlowList(host, port, projectName, null, sessionId).toString();
	}

	public JSONArray getFlowList(String host, String port, String projectName,
		...
		// If an error is encountered and the error code is "NotFound", it may
		// be a case that the project has not been
	...
	private boolean inSelected(Object obj, List<String> selectedFlows)
	...
	public String listAllFlows(String host, String port, String sessionId)
			throws Exception {
		JSONArray rtn = null;
		...
		if (projList != null) {
			...
			for (int i = 0; i < projList.length(); i++) {
				...
				JSONArray flowList;
				try {
					String sFlowList = listFlows(host, port,
							(String) proj.get("name"), sessionId);
	...
	// Imports Flow metadata, creates WSDL and generate service (service =
	// project, flows = operations)
	public void importFlows(String host, String port, String userName,
			String password, String domain, String projectName,
			List<String> selectedFlows, String sessionId) throws Exception {

		JSONArray flowList = getFlowList(host, port, projectName,
				selectedFlows, sessionId);
		...
		wstMgr.buildRestService(projectName, flowNames, inputParms, url,
				"POST", "application/x-www-form-urlencoded", "result", xsd,
				null, true);
	...
	private void createWarpPropertyFile(String projectName, String host,
			String port, String userName, String password, String domain)
			throws Exception {
		...
		File destdir = designServiceManager.getServiceRuntimeDirectory(
				projectName).getFile();

		File f = new File(destdir, projectName
	...
	private List<List<RESTInputParam>> getInputParms(JSONArray flowList)
			throws JSONException {
		...
		for (int i = 0; i < flowList.length(); i++) {
			...
			if (flow.has("args"))
				args = (JSONObject) flow.get("args");

			if (args != null && args.has("field"))
				fields = convertToJSONArray(args.get("field"));
			if (args != null && args.has("variable"))
				variables = convertToJSONArray(args.get("variable"));

			/*
			 * param = new RESTInputParam(); param.setName("sessionid");
			 * param.setType("string"); flowParams.add(param);
			 * 
			 * param = new RESTInputParam(); param.setName("project");
			 * param.setType("string"); flowParams.add(param);
			 * 
			 * param = new RESTInputParam(); param.setName("flow");
			 * param.setType("string"); flowParams.add(param);
			 */
	...
	private String completeXsd(String xsdBody, JSONObject flow)
			throws Exception {
		String flowName = (String) flow.get("name");
