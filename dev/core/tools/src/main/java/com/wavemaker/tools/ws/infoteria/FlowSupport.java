...
public class FlowSupport {
    ...
    public void importFlows(String host, String port, String userName, String password, String domain, String projectName,
        ...
        wstMgr.buildRestService(projectName, flowNames, inputParms, url, "POST", "application/x-www-form-urlencoded", "result", xsd, null, true);
    ...
    private String buildXsd(JSONArray flowList) throws Exception {
        StringBuffer sb = new StringBuffer(this.xsdHeader);

        for (int i = 0; i < flowList.length(); i++) {
            JSONObject flow = (JSONObject) flowList.get(i);
            String flowXsd = completeXsd(this.xsdBody, flow);
            sb.append(flowXsd);
            sb.append("\n");
        }

        sb.append(this.xsdTrailer);

        return sb.toString();
    }

    private void createWarpPropertyFile(String projectName, String host, String port, String userName, String password, String domain)
        throws Exception {
        ...
        File destdir = this.designServiceManager.getServiceRuntimeDirectory(projectName).getFile();
    ...
    private List<List<RESTInputParam>> getInputParms(JSONArray flowList) throws JSONException {
        ...
        for (int i = 0; i < flowList.length(); i++) {
            ...
            /*
             * param = new RESTInputParam(); param.setName("sessionid"); param.setType("string"); flowParams.add(param);
             * 
             * param = new RESTInputParam(); param.setName("project"); param.setType("string"); flowParams.add(param);
             * 
             * param = new RESTInputParam(); param.setName("flow"); param.setType("string"); flowParams.add(param);
             */
    ...
    private String completeXsd(String xsdBody, JSONObject flow) throws Exception {
        ...
        for (int j = 0; j < fields.length(); j++) {
            ...
            String rec = this.recElement.replaceAll("elem_name_", name);
