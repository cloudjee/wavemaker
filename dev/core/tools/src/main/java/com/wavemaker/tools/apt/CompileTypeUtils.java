...
public class CompileTypeUtils {
	...
	public static FieldDefinition buildFieldDefinition(
			ProcessingEnvironment processingEnv, TypeState typeState,
		...
		def.setTypeDefinition(buildTypeDefinition(processingEnv, typeState,
				type));

		if (type.getKind() == TypeKind.ARRAY
				|| (type.getKind() == TypeKind.DECLARED && isCollection(
						processingEnv, (DeclaredType) type))) {
			def.setArrayTypes(buildArrayTypesList(processingEnv, typeState,
					type, new ArrayList<ListTypeDefinition>()));
		}

		return def;
	}

	public static List<ListTypeDefinition> buildArrayTypesList(
			ProcessingEnvironment processingEnv, TypeState typeState,
			TypeMirror type, ArrayList<ListTypeDefinition> listTypes) {

		if (typeState.isTypeKnown(type.toString())) {
			listTypes.add((ListTypeDefinition) typeState.getType(type
					.toString()));
		} else {
		...
		TypeMirror componentType = componentTypeForArrayOrCollection(
				processingEnv, type);
		if (componentType.getKind() == TypeKind.ARRAY
				|| (componentType.getKind() == TypeKind.DECLARED && isCollection(
						processingEnv, (DeclaredType) componentType))) {
			return buildArrayTypesList(processingEnv, typeState, componentType,
					listTypes);
		} else {
			return listTypes;
		}
	...
	private static TypeMirror componentTypeForArrayOrCollection(
			ProcessingEnvironment processingEnv, TypeMirror type) {
		if (type.getKind() == TypeKind.ARRAY) {
	...
	public static TypeDefinition buildTypeDefinition(
		...
		case DECLARED:
			return typeDefForObject(processingEnv, typeState,
					(DeclaredType) type);

		case ARRAY:
			return buildTypeDefinition(processingEnv, typeState,
		...
		default:
			throw new WMRuntimeException("Cannot convert type "
					+ type.toString() + " to a TypeDefinition.");
		}
	}

	public static TypeDefinition typeDefForPrimitive(
			ProcessingEnvironment processingEnv, TypeState typeState,
			TypeMirror type) {
	...
	public static TypeDefinition typeDefForObject(
			ProcessingEnvironment processingEnv, TypeState typeState,
			DeclaredType type) {
		if (isPrimitiveWrapper(processingEnv, type)) {
			return typeDefForPrimitive(processingEnv, typeState, type);
		} else if (isCollection(processingEnv, type)) {
			return buildTypeDefinition(processingEnv, typeState, type
	...
	public static TypeDefinition typeDefForBean(
			ProcessingEnvironment processingEnv, TypeState typeState,
			DeclaredType type) {
		if (typeState.isTypeKnown(type.toString())) {
			return typeState.getType(type.toString());
	...
	public static TypeDefinition typeDefForEnum(
			ProcessingEnvironment processingEnv, TypeState typeState,
			DeclaredType type) {
	...
	public static TypeDefinition typeDefForMap(
			ProcessingEnvironment processingEnv, TypeState typeState,
			DeclaredType type) {
		...
		if (isProperties(processingEnv, type)) {
			...
			def.setKeyFieldDefinition(buildFieldDefinition(processingEnv,
					typeState, type.getTypeArguments().get(0), null));
			def.setValueFieldDefinition(buildFieldDefinition(processingEnv,
					typeState, type.getTypeArguments().get(1), null));
		} else {
	...
	public static boolean isCollection(ProcessingEnvironment processingEnv,
		...
		DeclaredType collectionType = types.getDeclaredType(
	...
	public static boolean isMap(ProcessingEnvironment processingEnv,
			DeclaredType type) {
		...
		Types types = processingEnv.getTypeUtils();
		Elements elements = processingEnv.getElementUtils();
		DeclaredType mapType = types.getDeclaredType(elements
				.getTypeElement(Map.class.getCanonicalName()), type
				.getTypeArguments().toArray(new TypeMirror[0]));
		return types.isSubtype(type, mapType);
	}

	public static boolean isProperties(ProcessingEnvironment processingEnv,
			DeclaredType type) {
		int numTypeArgs = type.getTypeArguments().size();
		if (numTypeArgs != 0 && numTypeArgs != 2) {
			return false;
		...
		Types types = processingEnv.getTypeUtils();
		Elements elements = processingEnv.getElementUtils();
		DeclaredType propertiesType = types.getDeclaredType(elements
				.getTypeElement(Properties.class.getCanonicalName()));
		return types.isSubtype(type, propertiesType);
	}

	public static boolean isEnum(ProcessingEnvironment processingEnv,
			DeclaredType type) {
		Types types = processingEnv.getTypeUtils();
		Elements elements = processingEnv.getElementUtils();
		return types
				.isSubtype(type,
						elements.getTypeElement(Enum.class.getCanonicalName())
								.asType());
	}

	public static boolean isPrimitiveWrapper(
			ProcessingEnvironment processingEnv, TypeMirror type) {
		Types types = processingEnv.getTypeUtils();
		Elements elements = processingEnv.getElementUtils();
		return types.isSubtype(type,
				elements.getTypeElement(Number.class.getCanonicalName())
						.asType())
				|| types.isSubtype(
						type,
						elements.getTypeElement(
								CharSequence.class.getCanonicalName()).asType())
				|| types.isSubtype(
						type,
						elements.getTypeElement(
								Boolean.class.getCanonicalName()).asType())
				|| types.isSubtype(
						type,
						elements.getTypeElement(
								Character.class.getCanonicalName()).asType())
				|| types.isSubtype(type,
						elements.getTypeElement(Byte.class.getCanonicalName())
								.asType());
	}

	private static DeclaredType declaredTypeForName(
			ProcessingEnvironment processingEnv, String className) {
		Types types = processingEnv.getTypeUtils();
		Elements elements = processingEnv.getElementUtils();
		return types.getDeclaredType(elements.getTypeElement(className));
	}

	private static class JavaBeanScanner extends
			ElementScanner6<TypeState, TypeState> {
		...
		public TypeState scan(Element element, TypeState typeState) {
			return element
					.accept(new JavaBeanVisitor(processingEnv), typeState);
		}

	}

	private static class JavaBeanVisitor extends
			ElementKindVisitor6<TypeState, TypeState> {
		...
		@Override
		public TypeState visitExecutableAsMethod(ExecutableElement method,
				TypeState typeState) {

			if (!method.getSimpleName().toString().endsWith("Class")) {

				ObjectReflectTypeDefinition def = (ObjectReflectTypeDefinition) typeState
						.getType(method.getEnclosingElement().asType()
								.toString());
				if (def != null) {
					...
					if (!def.getFields().containsKey(propName)) {
						if (isGetter(method)) {
							def.getFields().put(
									propName,
									CompileTypeUtils.buildFieldDefinition(
											processingEnv, typeState,
											method.getReturnType(), propName));
						} else if (isSetter(method)) {
							def.getFields().put(
									propName,
									CompileTypeUtils.buildFieldDefinition(
											processingEnv, typeState, method
													.getParameters().get(0)
													.asType(), propName));
						}
					}
