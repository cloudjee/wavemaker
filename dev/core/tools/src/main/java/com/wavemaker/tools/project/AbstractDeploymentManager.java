...
public abstract class AbstractDeploymentManager implements DeploymentManager {
    ...
    public FileUploadResponse importFromZip(MultipartFile file) throws IOException {
        ...
        try {
            ...
            Resource projectFolder = com.wavemaker.tools.project.ResourceManager.unzipFile(this.fileSystem, outputFile);
            ...
            // (__MACOS, .svn, etc...)
            List<Resource> listings = this.fileSystem.listChildren(projectFolder);
            if (listings.size() == 1) {
    ...
    public boolean undeployTheme(String themename) throws IOException {
        ...
        return this.fileSystem.deleteFile(packagesDir);
    }
    ...
    public String[] listThemes() throws IOException {
        ...
        Resource themesDir = this.fileSystem.getCommonDir().createRelative(THEMES_DIR);

        List<Resource> themesDirFiles = this.fileSystem.listChildren(themesDir);

        Resource themesFolder = this.fileSystem.getStudioWebAppRoot().createRelative("lib/wm/base/widget/themes/");

        List<Resource> widgetThemeFiles = this.fileSystem.listChildren(themesFolder, new ResourceFilter() {
    ...
    public void copyTheme(String oldName, String newName) throws IOException {
        ...
        Resource newFile = this.fileSystem.getCommonDir().createRelative(THEMES_DIR + newName);
