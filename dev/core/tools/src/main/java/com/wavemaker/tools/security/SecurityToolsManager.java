...
public class SecurityToolsManager {
	...
	public SecurityToolsManager(ProjectManager projectMgr,
	 ...
	 */
	private Beans getAcegiSpringBeansTemplate() throws JAXBException,
			IOException {
		ClassPathResource securityTemplateXml = new ClassPathResource(
				ACEGI_SPRING_TEMPLATE_CLASSPATH);
		Reader reader = new InputStreamReader(
				securityTemplateXml.getInputStream());
	...
	public Resource getAcegiSpringFile() {
		return getAcegiSpringFile(projectMgr.getCurrentProject());
	}

	private Resource getAcegiSpringFile(Project currentProject) {
		try {
			return currentProject.getWebInf().createRelative(
					ACEGI_SPRING_FILENAME);
	 ...
	 * 
	 * @param create
	 *            If this is set to true, it will create the Spring file using
	 *            the template if the Acegi Spring file does not exist in
	 *            current project.
	 */
	private Beans getAcegiSpringBeans(boolean create) throws IOException,
			JAXBException {
		Project currentProject = projectMgr.getCurrentProject();
	 ...
	 */
	private void saveAcegiSpringBeans(Beans beans) throws JAXBException,
			IOException {
		Project currentProject = projectMgr.getCurrentProject();
		Resource securityXml = getAcegiSpringFile(currentProject);
	...
	public void registerSecurityService() {
		Set<Service> services = designServiceMgr.getServices();
		boolean found = false;
		...
		if (!found) {
			try {
				...
				Set<String> serviceIds = designServiceMgr.getServiceIds();
				int i = 1;
				while (serviceIds.contains(serviceId)) {
					serviceId = SecurityServiceDefinition.DEFAULT_SERVICE_ID
							+ i;
					i++;
				}
				SecurityServiceDefinition def = new SecurityServiceDefinition(
						serviceId);
				designServiceMgr.defineService(def);
	...
	public void registerJOSSOSecurityService() {
		...
		if (!found) {
			try {
				...
				int i = 1;
				while (serviceIds.contains(serviceId)) {
					serviceId = JOSSOSecurityServiceDefinition.DEFAULT_SERVICE_ID
							+ i;
					i++;
				}
				JOSSOSecurityServiceDefinition def = new JOSSOSecurityServiceDefinition(
						serviceId);
				designServiceMgr.defineService(def);
			} catch (LinkageError e) {
				throw new ConfigurationException(e);
	...
	public GeneralOptions getGeneralOptions() throws JAXBException, IOException {
		...
		options.setEnforceSecurity(SecuritySpringSupport
				.isSecurityEnforced(beans));
		options.setEnforceIndexHtml(SecuritySpringSupport
				.isIndexHtmlEnforced(beans));
		options.setDataSourceType(SecuritySpringSupport
				.getDataSourceType(beans));
		return options;
	}

	public void setGeneralOptions(boolean enforceSecurity,
			boolean enforceIndexHtml) throws IOException, JAXBException {
	...
	public void setGeneralOptions(boolean enforceSecurity) throws IOException,
			JAXBException {
	...
	public void setStandardOptions(boolean enforceSecurity) throws IOException,
			JAXBException {
		Beans beans = getAcegiSpringBeans(false);
		SecuritySpringSupport.setSecurityResources(beans, enforceSecurity,
				false);
	...
	public List<String> getJOSSOOptions() throws JAXBException, IOException {
		...
		Resource webXml = projectMgr.getCurrentProject().getWebXml();
	 ...
	 * 
	 * @param primaryRole
	 *            The security-role role-name
	 */
	public void setJOSSOOptions(String primaryRole) throws JAXBException,
			IOException {
		boolean exists = false;
		designServiceMgr.getDeploymentManager().generateRuntime();
	...
	public void configDemo(DemoUser[] demoUsers) throws JAXBException,
			IOException {
		Beans beans = getAcegiSpringBeans(true);
		SecuritySpringSupport.updateAuthProviderUserDetailsService(beans,
				SecuritySpringSupport.IN_MEMORY_DAO_IMPL_BEAN_ID);
	...
	public void configDatabase(String modelName, String tableName,
			String unameColumnName, String uidColumnName,
			String uidColumnSqlType, String pwColumnName,
	...
	public List<String> testRolesByUsernameQuery(
			DataModelConfiguration dataModel, String query, String username) {
	...
	private static class JdbcDaoImplTest extends JdbcDaoImpl {
		...
		public List<String> loadUserRolesByUsername(String username) {
			List<GrantedAuthority> dbAuths = authoritiesByUsernameMapping
					.execute(username);
			...
			for (GrantedAuthority dbAuth : dbAuths) {
				...
				if (dbAuthString.startsWith(SecuritySpringSupport.ROLE_PREFIX)) {
					userRoles.add(dbAuthString
							.substring(SecuritySpringSupport.ROLE_PREFIX
									.length()));
				}
			}
			return userRoles;
	...
	/**
	 * Function for backwards compatibility (for functions that call this
	 * function without the DB authorization parameters)
	 * 
	 * @param ldapUrl
	 * @param managerDn
	...
	public void configLDAP(String ldapUrl, String managerDn,
		...
		configLDAP(ldapUrl, managerDn, managerPassword, userDnPattern,
	...
	public void configLDAP(String ldapUrl, String managerDn,
			...
			String roleUsername, String roleProperty, String roleQuery,
			String roleProvider) throws IOException, JAXBException {
		Beans beans = getAcegiSpringBeans(true);
	...
	/**
	 * Tests if the LDAP connection could be established successfully using the
	 * supplied parameters. An exception will be thrown if it can't connect to
	 * the LDAP server.
	 * 
	 * @param ldapUrl
	 *            The LDAP URL like "ldap://localhost:389/dc=wavemaker,dc=com"
	 * @param managerDn
	 *            The manager DN
	 * @param managerPassword
	 ...
	 */
	public static void testLDAPConnection(String ldapUrl, String managerDn,
			String managerPassword) {
		DefaultInitialDirContextFactory dirContext = new DefaultInitialDirContextFactory(
				ldapUrl);
	...
	public void setJOSSORoles(List<String> roles) throws IOException,
			JAXBException {
		// not implemented
	}
	 ...
	 * 
	 * @param urlMap
	 *            The new object definition source map. Replaces existing map
	 * @throws IOException
	 * @throws JAXBException
	 */
