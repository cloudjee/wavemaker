...
public class WebServiceSpringSupport {
	 ...
	 * 
	 * @param designServiceMgr
	 *            The <code>DesignServiceManager</code>.
	 * @param serviceId
	 ...
	 * @throws IOException
	 */
	public static BindingProperties getBindingProperties(
			FileService fileService, DesignServiceManager designServiceMgr,
			String serviceId) throws JAXBException, IOException {
		...
		File springFile = new File(serviceRuntimeDirectory,
				service.getSpringFile());
		if (springFile.exists()) {
			Beans beans = SpringConfigSupport.readBeans(new FileSystemResource(
					springFile), fileService);
			...
			if (property != null) {
				...
				if (prop4 != null) {
					bindingProperties.setRequestTimeout(Integer.parseInt(prop4
							.getValue()));
				}
			}
	 ...
	 *            The binding properties to be set.
	 * @throws JAXBException
	 * @throws IOException
	 */
	public static void setBindingProperties(FileService fileService,
		...
		if (bindingProperties != null) {
			...
			SpringConfigSupport.writeBeans(beans, new FileSystemResource(
					springFile), fileService);
		}
	}
}
