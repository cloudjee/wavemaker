...
public class DesignServiceManager {
	...
	public static void generateSpringServiceConfig(String serviceId,
			...
			Resource serviceBeanFile, Project project) throws JAXBException,
			IOException {
		...
		Bean serviceWireBean = generateServiceWireBean(designServiceType,
				serviceId);
	 ...
	 */
	public static Bean generateServiceWireBean(
			DesignServiceType designServiceType, String serviceId) {

		// serviceType bean
		Bean serviceTypeBean = new Bean();
		serviceTypeBean.setClazz(designServiceType.getServiceType().getClass()
				.getName());
	 ...
	 * @throws JAXBException
	 */
	public void deleteServiceSmd(String serviceId) throws IOException,
			NoSuchMethodException {
	...
	public void defineService(ServiceDefinition serviceDef,
			DataModelConfiguration cfg, String username, String password) { // salesforce
		this.cfg = cfg;
	 ...
	 */
	public void defineService(ServiceDefinition serviceDef, String username,
			String password) {
		...
		try {
			...
			if (serviceDef instanceof ReflectServiceDefinition) {
				service.setClazz(((ReflectServiceDefinition) serviceDef)
						.getServiceClass());
			}
		...
		if (svcId.equals(CommonConstants.SALESFORCE_SERVICE)
				&& username != null) { // salesforce
			SalesforceHelper.setupSalesforceSrc(projectManager, username,
					password);
		}
	...
	public Resource getServicesDir() {
		try {
			return getProjectManager().getCurrentProject().getProjectRoot()
					.createRelative(SERVICES_DIR);
	...
	public Resource getServiceBeanXml(String serviceId) {
		try {
			return getProjectManager().getCurrentProject().getProjectRoot()
					.createRelative(getServiceBeanRelative(serviceId));
		} catch (IOException ex) {
			throw new WMRuntimeException(ex);
		}
	}
	...
	public SortedSet<String> getServiceIds() {
		...
		if (servicesDir.exists()) {
			for (Resource child : studioConfiguration.listChildren(servicesDir)) {
				if (org.springframework.util.StringUtils
						.getFilenameExtension(child.getFilename()) == null) {
	...
	public DesignServiceType getDesignServiceType(String typeId) {
		...
		throw new WMRuntimeException(
				MessageResource.NO_DESIGN_SERVICE_TYPE_FOUND, typeId);
	}
	 ...
	 * 
	 * @param services
	 *            The list of services to update; if null, all services' SMDs
	 *            are generated.
	 * 
	 * @throws JAXBException
	 * @throws IOException
	...
	private void generateRuntimeConfiguration(SortedSet<Service> services)
		...
		ConfigurationCompiler.generateTypes(project,
				ConfigurationCompiler.getTypesFile(project), allServices,
	 ...
	 * @param serviceDef
	 */
	public void updateServiceTypes(Service service,
			ServiceDefinition serviceDef, String username, String password) { // salesforce
		...
		for (TypeDefinition et : elementTypes) {
			...
			if (et instanceof ObjectTypeDefinition) {
				...
				for (Map.Entry<String, FieldDefinition> entry : otd.getFields()
						.entrySet()) {
					...
					if (0 != entry.getKey().compareTo(
						...
						throw new WMRuntimeException("key: " + entry.getKey()
								+ " != " + entry.getValue().getName());
					}
					...
					if (null != entry.getValue().getTypeDefinition()) {
						e.setTypeRef(entry.getValue().getTypeDefinition()
								.getTypeName());
					}
					...
					for (OperationEnumeration oe : entry.getValue()
							.getRequire()) {
						e.getRequire().add(oe);
					}
					for (OperationEnumeration oe : entry.getValue()
	...
	public ClassLoader getServiceRuntimeClassLoader(String sid) {
		...
		try {
			return ClassLoaderUtils.getClassLoaderForFile(
					getServiceRuntimeDirectory(sid).getFile(), projectManager
							.getCurrentProject().getWebInfClasses().getFile());
		} catch (IOException ex) {
			throw new WMRuntimeException(ex);
		}
	}
	...
	protected void doUpdateOperation(ServiceOperation so, Service service,
		...
		if (null != so.getReturnType()) {
			...
			if (pwsServiceModifierBeanFactory == null
					|| sd.getPartnerName() == null) {
				retType = so.getReturnType();
			} else {
				IPwsServiceModifier serviceModifier = pwsServiceModifierBeanFactory
		...
		if (serviceDefFile.exists()) {
			try {
				...
				if (null == ret.getId() || "" == ret.getId()) {
					throw new WMRuntimeException(
							MessageResource.INVALID_SERVICE_DEF_NO_ID,
							serviceId);
				}
			} catch (JAXBException e) {
				throw new WMRuntimeException(
						MessageResource.ERROR_LOADING_SERVICEDEF, e, serviceId,
						serviceDefFile, e.getMessage());
			}
		}
	...
	public static Service loadServiceDefinition(Resource serviceDefXml)
			throws JAXBException {
	...
	public void saveServiceDefinition(String serviceId) {
		...
		if (serviceId.equals(CommonConstants.SALESFORCE_SERVICE)) {
			...
			if (oldService != null) {
				...
				List<DataObject> dataObjects = service.getDataobjects()
						.getDataobject();
				for (DataObject dobj : oldService.getDataobjects()
						.getDataobject()) {
	...
	public void setPwsServiceModifierBeanFactory(
			PwsServiceModifierBeanFactory pwsServiceModifierBeanFactory) {
	...
	public void setDefaultServiceModifier(
			IPwsServiceModifier defaultServiceModifier) {
