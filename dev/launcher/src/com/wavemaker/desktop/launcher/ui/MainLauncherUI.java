...
public class MainLauncherUI extends JFrame {
	...
	public MainLauncherUI(String version, TomcatConfig config) {
		...
		try {
			BufferedImage source = javax.imageio.ImageIO
					.read(MainLauncherUI.class
							.getResourceAsStream("wavemaker_small.png"));
			if (source != null) {
			/*
	...
	private void initOptions() {
		...
		final Preferences prefs = Preferences
				.userNodeForPackage(MainLauncherUI.class);
		...
		// Server Ports
		this.servicePortField = new JTextField(Integer.toString(this.config
				.getServicePort()));
		this.servicePortField.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent arg0) {
				try {
					...
					servicePortField.setText(Integer.toString(config
							.getServicePort()));
				}
			}
		});
		...
		this.shutdownPortField = new JTextField(Integer.toString(this.config
				.getShutdownPort()));
		this.shutdownPortField.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent arg0) {
				try {
					...
					shutdownPortField.setText(Integer.toString(config
							.getShutdownPort()));
				}
			}
		});
		...
		saveButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (JOptionPane.showConfirmDialog(JOptionPane.getRootFrame(),
					...
					prefs.putBoolean(OPTION_ENABLE_DEBUG, debugCheckbox
							.isSelected());
					prefs.putBoolean(OPTION_ENABLE_LIVELAYOUT, noLiveCheckbox
							.isSelected());
					try {
						Main.setDefaultTomcatConfiguration(config);
					} catch (IOException e) {
						e.printStackTrace();
						JOptionPane.showConfirmDialog(JOptionPane
								.getRootFrame(),
		...
		this.config.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				if (evt.getPropertyName() == TomcatConfig.PROPERTY_SHUTDOWN_PORT) {
					shutdownPortField.setText(Integer.toString(config
							.getShutdownPort()));
					shutdownPortStatus.setText(Integer.toString(config
							.getShutdownPort()));
				} else if (evt.getPropertyName() == TomcatConfig.PROPERTY_SERVICE_PORT) {
					servicePortField.setText(Integer.toString(config
							.getServicePort()));
					servicePortStatus.setText(Integer.toString(config
							.getServicePort()));
				}
			}
		});
						...
						appServer.getLauncher().addLifecycleListener(
								new LifecycleListener() {
									public void lifecycleEvent(
										...
										} else if (Lifecycle.BEFORE_START_EVENT
												.equals(event.getType())) {
											serverStatus.setText("Starting");
										} else if (Lifecycle.START_EVENT
												.equals(event.getType())) {
											serverStatus.setText("Started");
										} else if (Lifecycle.AFTER_START_EVENT
												.equals(event.getType())) {
