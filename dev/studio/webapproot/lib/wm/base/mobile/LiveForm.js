 ...
 */
dojo.provide("wm.base.mobile.LiveForm");
...
wm.getMatchingFormWidgets = function(inForm, inMatch) {
	...
	wm.forEach(inForm.widgets, function(w) {
			if (inMatch(w))
				match.push(w);
	    if ((wm.isInstanceType(w, wm.mobile.Container) && !(wm.isInstanceType(w, wm.mobile.LiveFormBase))))
				match = match.concat(wm.getMatchingFormWidgets(w, inMatch));
		});
		return match;
};

wm.mobile.getParentForm = function(inWidget) {
	var w = inWidget.parent;
	var r = inWidget.getRoot();
	r = r && r.root;
	while (w && w != r) {
	        if (wm.isInstanceType(w, wm.mobile.FormContainer)) {
			return w;
		}
		w = w.parent;
	}
...
wm.getFormField = function(inWidget) {
	var a = [], w = inWidget;
        while (w && !(wm.isInstanceType(w, wm.mobile.LiveForm))) {
		if (w.formField)
			a.unshift(w.formField);
		w = wm.mobile.getParentForm(w);
	}
	return a.join('.');
}

dojo.declare("wm.mobile.FormContainer", wm.mobile.Container, {
	editorHeight: wm.mobile.AbstractEditor.prototype.height,
	editorWidth: "100%",
	captionSize: "120px",
	captionAlign: "right",
	...
	getEditorsArray: function() {
		return wm.getMatchingFormWidgets(this, function(w) {
		    return (wm.isInstanceType(w, wm.mobile.AbstractEditor));
		});
	},
...
/**
	Base class for {@link wm.mobile.LiveForm}.
	@name wm.mobile.LiveFormBase
	@class
	@extends wm.Container
	@noindex
*/
dojo.declare("wm.mobile.LiveFormBase", wm.mobile.FormContainer, {
	/** @lends wm.mobile.LiveFormBase.prototype */
	...
	*/
	...
	getLiveVariable: function() {
		...
		*/
		...
		while (s && s instanceof wm.Variable) {
	...
	populateEditors: function() {
	    ...
	    dojo.forEach(this.getFormEditorsArray(), dojo.hitch(this, function(e) {
			if (wm.isInstanceType(e, wm.mobile.LiveFormBase)) {
				wm.fire(e, "populateEditors");
			} else {
                            if (e instanceof wm.mobile.Lookup && (!e.dataSet || !e.dataSet.type)) 
	...
	populateDataOutput: function() {
	    ...
	    d.isList = false; // sanity check...
	    dojo.forEach(this.getFormEditorsArray(), dojo.hitch(this, function(e) {
			if (wm.isInstanceType(e, wm.mobile.LiveFormBase)) {
                            wm.fire(e, "populateDataOutput"); // this.dataOutput will be updated via bindings once e has been updated
			}else if (e.formField) {
			    if (e instanceof wm.mobile.Lookup) {
				d.setValue(e.formField, e.selectedItem);
			    } else {
				d.setValue(e.formField, e.getDataValue());
			    }
			} else if (this instanceof wm.mobile.RelatedEditor) {
	...
	*/
	...
	getRelatedEditorsArray: function(inContainer) {
		return wm.getMatchingFormWidgets(this, function(w) {
			return (wm.isInstanceType(w, wm.mobile.RelatedEditor));
		});
	},
	...
	_getEditorBindSource: function(inSourceId) {
		...
		if (wm.isInstanceType(v, wm.mobile.RelatedEditor))
			return v;
	},
	...
	getViewDataIndex: function(inFormField) {
	 ...
	 var result = true;
	        dojo.forEach(this.getFormEditorsArray(), function(e) {
		  if (wm.isInstanceType(e, wm.mobile.RelatedEditor)) {
...
dojo.declare("wm.mobile.SimpleForm", wm.mobile.LiveFormBase, {
});
...
/**
	...
	@name wm.mobile.LiveForm
	@class
	@extends wm.mobile.LiveFormBase
*/
dojo.declare("wm.mobile.LiveForm", wm.mobile.LiveFormBase, {
	/**
		@lends wm.mobile.LiveForm.prototype
	*/
...
/*
		...
		*/
		...
		wm.getMatchingFormWidgets(this, function(w) {
		    if (w instanceof wm.mobile.AbstractEditor ||
			w instanceof wm.mobile.RelatedEditor) {
			w.validate();
		    }});
	    }
	},
...
/*
	...
	*/
	...
	_canChangeEditorReadonly: function(inOperations, inEditor, inForm, inReadonly) {
	    if (( wm.isInstanceType(inEditor, wm.mobile.AbstractEditor)) && inEditor.formField) {
			var
...
/* please use saveDataIfValid instead of validateBeforeSave
	...
	*/
	deleteData: function() {
		...
		app.confirm(this._formMessages.confirmDelete, f);
	...
	getSubFormsArray: function() {
		...
		for (var i in this.widgets) {
			w = this.widgets[i];
			if (wm.isInstanceType(w, wm.mobile.LiveForm)) {
				forms.push(w);
