...
dojo.provide("wm.base.mobile.BusyButton");

dojo.declare("wm.mobile.BusyButton", wm.mobile.Button, {
	...
	if (this.iconUrl) {
	    ...
	    if (!img) 
		img = dojo.create("img", {}, this.domNode);
	    img.src =  root + url;
	} else if (img) {
	    dojo.destroy(img);
	}
	 ...
	 */
        onSuccess: function() {
	    this.setIconUrl(this.iconSuccessUrl, true);
	    if (this.iconSuccessUrl) {
		wm.job("ClearBusyButton" + this.name, 5000, 
		       dojo.hitch(this, function() {
			       this.setIconUrl("", true);
			   }));
	    }
	...
	onError: function(inError) {
	    ...
	    this.setIconUrl(this.iconErrorUrl, true);
	    this.disconnectEvent("onSuccess");
	    this.disconnectEvent("onError");
	    this.disconnectEvent("onResult");
	},
...
wm.Object.extendSchema(wm.mobile.BusyButton, {
      iconUrl: {ignore: 1},
...
wm.mobile.BusyButton.description = "A button that indicates when its ServiceVariable/LiveVariable is processing the button's action";
