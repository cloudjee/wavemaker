 ...
 */
dojo.provide("wm.base.mobile.RelatedEditor_design");
dojo.require("wm.base.mobile.RelatedEditor");

wm.Object.extendSchema(wm.mobile.RelatedEditor, {
	dataSet: {ignore: 1},
...
wm.mobile.RelatedEditor.extend({
	...
	_hasAncestorEditingMode: function(inModes) {
		var p = this.parent;
		while (p && p instanceof wm.mobile.RelatedEditor) {
	...
	bindFormEditors: function(inRecurse) {
		...
		dojo.forEach(editors, function(e) {
			this._bindEditor(e);
			if (inRecurse && e instanceof wm.mobile.RelatedEditor)
