 ...
 */
dojo.provide("wm.base.mobile.Container");
...
/**
	...
	@extends wm.Control
*/
wm.define("wm.mobile.Container", wm.Control, {
	...
	init: function() {
		...
		this._touchScroll = new dojox.mobile.scrollable();
		this._touchScroll.resizeView = dojo.hitch(this, "touchScrollResizeView");
		var div = document.createElement("div");
		div.className = "DojoTouchScroller";
		this.domNode.appendChild(div);
		while (this.domNode.childNodes.length > 1)
		    div.appendChild(this.domNode.childNodes[0]);
		this.updateScroller();
/*
		...
		*/
		...
		wm.job(this.getRuntimeId() + ".updateScroller", 10, dojo.hitch(this, function() {
		    if (!this.domNode.style.display) {
		    this._touchScroll.init({
			domNode: this.domNode,
			containerNode: this.domNode.firstChild,
			fixedHeaderHeight: 0,
			fixedFooterHeight: document.body.clientHeight - dojo.coords(this.domNode).y,
			_appFooterHeight: 0
		    });
        ...
        touchScrollResizeView: function(e) {
	    if (this.domNode.style.display == "none") return;
	    this._touchScroll._appFooterHeight = this._touchScroll.fixedFooterHeight && !this._touchScroll.isLocalFooter ? this._touchScroll.fixedFooterHeight : 0;
	    this._touchScroll.containerNode.style.paddingTop = this._touchScroll.fixedHeaderHeight + "px";

	    wm.job(this.getRuntimeId() + "touchScrollResizeView", 10, dojo.hitch(this, function() {
		var node = this.domNode;
		var style = node.style;
		style.height = this.bounds.h + "px";
		this._touchScroll.resetScrollBar();
	    }));
	},
	postInit: function() {
		this.inherited(arguments);
	    if (this.fitToContentWidth)
		this.setWidth("50px"); // random number
	    if (this.fitToContentHeight)
		this.setHeight("50px"); // random number
/*
	...
	addControl: function(inControl) {
	    ...
	    if (this.dom && this.isRelativePositioned)
		this.dom.append(inControl.dom);
	},
    removeControl: function(inControl, noNewParent) {
	    if (this.dom && noNewParent && this.isRelativePositioned)
	...
	insertControl: function(inControl, inIndex) {
	    ...
	    if (this.dom && this.isRelativePositioned)
		this.dom.insert(inControl.dom, inIndex);
	},
	...
	getInvalidWidget: function() {
		...
		} else if (wm.isInstanceType(w,wm.mobile.Container)) {
    ...
    getLock: function() {return this.lock;}, // Control calls getLock :-(
    getFreeze: function() {return this.freeze;}, // Control calls getLock :-(
...
wm.mobile.Container.extend({
/*
        generateLoadingHtml: function() {
	    var cssText = this.domNode.style.cssText;
	    var widgets = ["<div style='" + cssText + "'  class='" + this.domNode.className + "'>"];
	    for (var i = 0; i < this.c$.length; i++) {
		widgets = widgets.concat(this.c$[i].generateLoadingHtml());
	    }
	    widgets.push("</div>");
	    return widgets;
	},
...
wm.Object.extendSchema(wm.mobile.Container, {
    ...
    scrollX: {group: "scrolling", order: 101, ignore: 1},
    scrollY: {group: "scrolling", order: 102, ignore: 1},
    touchScrolling: {group: "scrolling", order: 103, ignore: 1},
