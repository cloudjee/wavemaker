 ...
 */
dojo.provide("wm.base.mobile.RelatedEditor");
dojo.require("wm.base.mobile.LiveForm");

dojo.declare("wm.mobile.RelatedEditor", wm.mobile.LiveFormBase, {
        margin: "0",
	height:'26px',
	editingMode: "lookup",
	_lookupCache: null,
	...
	findLookup: function() {
		...
		for (var i = 0, e; (e=editors[i]); i++)
			if (e.display == "Lookup" || e instanceof wm.mobile.Lookup) // added e instanceof because e.display == undefined for my test case; not even clear where display is set.
				return e;
	},
	//===========================================================================
	...
	_getLiveForm: function() {
		...
		while (p && !(wm.isInstanceType(p, wm.mobile.LiveForm))) {
