 ...
 */
dojo.provide("wm.base.mobile.Panel");
...
*/
dojo.declare("wm.mobile.Panel", wm.mobile.Container, {
...
dojo.declare("wm.mobile.TitledPanel", wm.mobile.Panel, {
    //useDesignBorder: 0, // move this to a _design file if we ever create one
    freeze: true,
    classNames: "wmcontainer wmmobtitledpanel",
    title: "Panel Heading",
    labelWidget: null,
    ...
    init: function() {
	    ...
	    this.labelWidget = new wm.mobile.Label({_classes: {domNode: ["wmmobpaneltitle"]},
						    _dontRenderCss: true,
	    ...
	    this.labelWidget.domNode.style.border = "solid " + this.innerBorder + "px " + this.borderColor;
	    this.labelWidget.domNode.style.borderBottomWidth = "0px";
	    var innerBorder = String(this.innerBorder);
            innerBorder = this._parseExtents(innerBorder);
	    this.containerWidget = new wm.mobile.Container({
		_classes: {domNode: ["wmmobpanelcontainer"]},
...
/*
	...
	wm.mobile.Panel.prototype.setLayoutKind.call(this,"top-to-bottom");
	if (this.containerWidget) {
...
wm.mobile.Panel.extend({
    themeable: false,
    // backward-compatibility fixups
	afterPaletteDrop: function() {
		this.inherited(arguments);
	    if (this instanceof wm.mobile.TitledPanel) return;
...
wm.Object.extendSchema(wm.mobile.TitledPanel, {
...
wm.mobile.Panel.description = "A container for widgets.";
