			...
			try
			{
					...
					&& lastHtml.indexOf('<span') == -1 && lastHtml.indexOf('</span') == -1 )
				{
					...
					else
					{
						...
						// there's a bug while calculating lastHtml and when it's length is greater than 0, 
						// we see a background text in the js editor. For now, throwing an error so that 
						// it makes doHtmlOpti = false and we replace the entire textNode instead of replacing a fraction of text node.
