...
t=Object.create(t);if(type==='(string)'){if(jx.test(value)){warningAt("Script URL.",line,from);}}else if(type==='(identifier)'){if(option.nomen&&(value.charAt(0)==='_'||value.charAt(value.length-1)==='_')){warningAt("Unexpected '_' in '{a}'.",line,from,value);}}
...
if(xmode===x||xmode==='string'){if(xmode&&xmode!=='CDATA'&&ax.test(x)){warning("ADsafe string violation.",line,character);}
...
return it('(number)',t);}
switch(t){case'"':case"'":return string(t);case'//':if(src||(xmode&&!(xmode==='script'||xmode==='CDATA'))){warningAt("Unexpected comment.",line,character);}else if(xmode==='script'&&/\<\/script\>/i.test(s)){warningAt("Unexpected <\/script> in comment.",line,character);}else if((option.safe||xmode==='script')&&ax.test(s)){warningAt("Dangerous comment.",line,character);}
...
character+=l;s=s.substr(l);return it('(regex)',c);case'\\':c=s.charAt(0);if(c<' '){warningAt("Unexpected control character in regular expression.",line,from+l);}else if(c==='<'){warningAt("Unexpected escaped character '{a}' in regular expression.",line,from+l,c);}
...
function statements(begin){var a=[];if(begin){if(option.safe&&nexttoken.type!=='(string)'){warning('ADsafe violation: Missing "use subset cautious" statement.',nexttoken);}
if(nexttoken.type==='(string)'&&nexttoken.value.slice(0,10)==='use subset cautious'){advance();advance(';');}}
...
return a;}
function block(f){var a,b=inblock,s=scope;inblock=f;if(f){scope=Object.create(scope);}
...
if(!e){stack.push(t);}
break;case'</':advance('</');n=xmlword(true);if(!stack){error("Unexpected '{a}'.",nexttoken,closetag(n));}
...
countMember(e.value);if(!option.sub&&ix.test(e.value)){s=syntax[e.value];if(!s||!s.reserved){warning("['{a}'] is better written in dot notation.",e,e.value);}}}else if(!e||e.type!=='(number)'){if(option.safe){warning('ADsafe subscripting.');}}
...
return this;});blockstmt('try',function(){var b,e,s;block(false);if(nexttoken.id==='catch'){advance('catch');nonadjacent(token,nexttoken);advance('(');s=scope;scope=Object.create(s);e=nexttoken.value;if(nexttoken.type!=='(identifier)'){warning("Expected an identifier and instead saw '{a}'.",nexttoken,e);}else{addlabel(e,'unused');}
