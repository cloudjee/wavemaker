...
dojo.declare("ResourceManager", wm.Page, {
	...
	if (!target)
	    return;
	if (this.draggedItem.parent == target) return;
	...
	var _this = this;
	studio.resourceManagerService.requestAsync("renameFile", [this.draggedItem.getFullFilePath(), target.getFullFilePath() + "/" + this.draggedItem.getItemName()], 
						   function(response) {
						       if (!response) {
...
dojo.declare("wm.ResourcePalette", wm.Tree, {
	...
	dragDropPalette: function() {
	    ...
	    manager.setSelectedItem(droppedItem);
...
dojo.declare("wm.ResourceMover", wm.DragDropper, {
	...
	_findTarget: function(inHit, inWidget, inMargin) {
		...
		for (var i in ws) {
			w = ws[i];
			// Can't drop an item into itself.  
			if (w != this.info.control && 

			    // Can only drop onto an item that is a resourceDropTarget
			    w.resourceDropTarget && 

			    // can't drop an item into one of its children
			    !w.isDescendantOf(this.info.control) ) {
...
dojo.declare("wm.ResourceItem", wm.Container, {
	...
	this.itemName = this.itemName || "New_Item_" + manager.itemCount++;
	...
	var parent = this;
	while (parent.parent != null && parent.parent != window && parent != possibleParent)
    ...
    initFromJSON: function(json) {
	    ...
	    if (result && result[0] > manager.itemCount)
		manager.itemCount = parseInt(result[0])+1;
    },
    mousedown: function(e) {
	if (!this.isRoot)
	    this.drag(e);
	...
	var droppedItem = eval("new " + this.dragger.info.type + "()");
	target.addChild(droppedItem);
	studio.resourceManager.setSelectedItem(droppedItem);
	if (this.dragger.info.type == "wm.FolderResourceItem")
	    droppedItem.mkdir();
    },
    */
	...
	var _this = this;
	var manager = studio.resourcesPage.getComponent("resourceManager");
	studio.resourceManagerService.requestAsync("renameFile", [this.getFullFilePath(), newpath],
						   function(result) {
						       if (!result) {
    ...
    */
	...
	new wm.Label({parent: panel, 
		      ...
		      width: "100%",
		      height: "35px"});
	var b = new wm.Button({parent: panel,
			       caption: "Delete",		       
			       onclick: function() {_this.deleteItem();}});
	b.domNode.style.color = "black";
    },
	...
	if (this.showFileUplaod) {
	    ...
	    upload_edit.uploadBtn.domNode.style.color = "black";
	}	    
	if (hasFile) {
	    new wm.Label({parent: panel,
			  caption: "path:",
	...
	studio.resourceManagerService.requestAsync("renameFile", [".tmp/" + uploadedName, this.parent.getFullFilePath() + "/" + uploadedName],
						   function(result) {
	...
	//this.disconnect(resourceManager.resourceManager, "onSuccess", this, "finishFileUploadOnSuccess");
	if (!result) {
	    alert("Failed to place file");
	    return;
	}
	//this.reflow();
	this.resourcesCanvas.reflow();
	...
	var manager = studio.resourcesPage.getComponent("resourceManager");
 	var _this = this;
	studio.resourceManagerService.requestAsync("deleteFile", [ this.getFullFilePath()],
						   function(result) { 
						       if (!result) {
...
dojo.declare("wm.ZipResourceItem", wm.ResourceItem, {
	...
	var manager = studio.resourcesPage.getComponent("resourceManager");
	var _this = this;
	studio.resourceManagerService.requestAsync("unzipFile", [ this.getFullFilePath()],	
						   function(result) {
						       if (!result) {
							   alert("Failed to unzip the file");
							   return;
						       }
						       manager.loadResourcesData();
						   });
    }
});
...
dojo.declare("wm.FolderResourceItem", wm.ResourceItem, {
	...
	if (!this.saved) {
	    ...
	    var newfolder = this;
	    studio.resourceManagerService.requestAsync("createFolder", [ this.getFullFilePath()],	
						       function(result) {
							   if (!result) {
