...
dojo.provide("wm.base.widget.Dwim");
...
wm.DwimLayout = {
	...
	suggest: function(inContainer, inControl, ioRect) {
		...
		var b = {};
		//
		var dt = ioRect.t - info.t;
		var db = info.b - ioRect.t;
		var dy = Math.min(dt, db);
		var t = dt < db ? info.t : info.b;
		//
		var dl = ioRect.l - info.l;
		var dr = info.r - ioRect.l;
		var dx = Math.min(dl, dr);
		var l = dl < dr ? info.l : info.r;
		//
		if (dy < dx) {
			info = {
				l:info.l,
...
dojo.declare("wm.Dwim", wm.Container, {
	layout: wm.DwimLayout,
	setLayoutKind: function(inLayoutKind) {
	}
});

dojo.declare("wm.Abs", wm.Container, {
	layout: new wm.layout.Abs(),
	setLayoutKind: function(inLayoutKind) {
	}
});

wm.registerPackage(["Common", "DwimLayout", "wm.Dwim", "wm.base.widget.Dwim", "images/wm/panel.png", "A Dwim layout panel."]);
wm.registerPackage(["Common", "AbsLayout", "wm.Abs", "wm.base.widget.Dwim", "images/wm/panel.png", "An Abs layout panel."]);
