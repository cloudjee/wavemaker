...
public class StudioService extends ClassLoader {
	...
	public String closurecompile(String s) {
		...
		try {
			...
			String data = "output_format=json&output_info=errors&js_code="
	...
	public OpenProjectReturn openProject(String projectName) throws IOException {
		projectManager.openProject(projectManager.getUserProjectPrefix()
				+ projectName);
	...
	public String newProject(String projectName) throws IOException {
		...
		projectManager.newProject(projectManager.getUserProjectPrefix()
				+ projectName);
		return getWebPath();
	}
	...
	public void deleteProject(String projectName) throws IOException {
		...
		try {
			deploymentManager.testRunClean(
					projectManager.getProjectDir(pName, true).getURI()
							.toString(), pName);
	...
	public void copyProject(String sourceProjectName,
		...
		projectManager.copyProject(projectManager.getUserProjectPrefix()
				+ sourceProjectName, projectManager.getUserProjectPrefix()
	 ...
	 * activeGridHome, or eventually absolute. This will clobber existing files.
	 * 
	 * @param path
	 ...
	 * 
	 * @param path
	 *            The path to check (relative to the project root).
	 * @return True iff the path points to an existant file or directory.
	 */
	@ExposeToClient
	public boolean fileExists(String path) {
		try {
			return projectManager.getCurrentProject().fileExists(
					projectManager.getUserProjectPrefix() + path);
		} catch (IOException e) {
	...
	public void writeCommonFile(String path, String data) throws IOException {
		...
		if (writeFile.exists()) {
			String original = projectManager.getCurrentProject().readFile(
					writeFile);

			if (original.equals(data))
				return;
		}
		projectManager.getCurrentProject().writeFile(writeFile, data);
	}
	...
	public String getLatestPatches(String url) {
		...
		try {
			...
			/*
			 * int startDiv =
			 * str.indexOf("<div class=\"main layoutsubsection\">");
			 */
	...
	public String readWebFile(String path) throws IOException {
		...
		String newPath = ProjectConstants.WEB_DIR + "/"
				+ projectManager.getUserProjectPrefix() + path;
	...
	public String getStudioProjectType() throws IOException {
		...
		String versionFileContents = com.wavemaker.tools.project.LocalStudioConfiguration
				.getCurrentVersionInfoString();
	...
	public String getMainLog(int lines) throws IOException {
		Resource logDir = studioConfiguration.createPath(studioConfiguration.getWaveMakerHome(), "logs/");
		Resource logFile = logDir.createRelative("wm.log");
		return IOUtils.tail(logFile.getFile(), lines);
	}

	@ExposeToClient
	public java.util.Hashtable getLogUpdate(String filename, String lastStamp)
			throws IOException {
		File logFolder = new File(System.getProperty("catalina.home")
				+ "/logs/ProjectLogs", projectManager.getCurrentProject()
				.getProjectName());
		...
		if (lastStamp.length() > 0) {
			...
			Pattern newStuffPattern = Pattern.compile(
					"^.*START_WM_LOG_LINE\\s+" + lastStamp + "\\s+",
		...
		java.util.regex.Pattern htmlTagPattern = java.util.regex.Pattern
		...
		java.util.regex.Pattern p = java.util.regex.Pattern
				.compile("START_WM_LOG_LINE\\s+\\d+\\:\\d+\\:\\d+,\\d+");
	...
	/*
	 * Note this should probably be moved to javaServices.java, but I didn't
	 * want to spend the time to figure out how to access studioConfiguration
	 * from that file
	 */
	@ExposeToClient
	public String getJavaServiceTemplate(String templateName)
			throws IOException {
		return getProjectManager().getCurrentProject().readFile(studioConfiguration
				.getStudioWebAppRoot().createRelative("/app/templates/javaservices"+templateName));
	}
	...
	public FileUploadResponse uploadExtensionZip(MultipartFile file) {
		...
		File tmpDir;
		try {
			File webapproot = new File(WMAppContext.getInstance()
					.getAppContextRoot());

			String filename = file.getOriginalFilename();
			int dotindex = filename.lastIndexOf(".");
			String ext = (dotindex == -1) ? "" : filename
					.substring(dotindex + 1);
			if (dotindex == -1)
				throw new IOException("Please upload a zip file");
			else if (!ext.equals("zip")) {
				throw new IOException("Please upload a zip file, not a " + ext
						+ " file");
			}
			...
			System.out.println("OLD NAME:" + originalName + "; NEW NAME:"
					+ newName);
			FileOutputStream fos = new FileOutputStream(outputFile);
			IOUtils.copy(file.getInputStream(), fos);
			file.getInputStream().close();
			fos.close();
			File extFolder = com.wavemaker.tools.project.ResourceManager
					.unzipFile(getStudioConfiguration(), new FileSystemResource(outputFile)).getFile();
			...
			for (int i = 0; i < pages.length; i++) {
				IOUtils.copy(pages[i],
						new File(webapprootPages, pages[i].getName()));
			}

			/*
			 * Import the language files from the dictionaries folder and
			 * subfolder STATUS: DONE
			 */
			...
			for (int i = 0; i < languages.length; i++) {
				if (languages[i].isDirectory()) {
					System.out.println("GET LISTING OF "
							+ languages[i].getName());
					File[] languages2 = languages[i].listFiles();
					for (int j = 0; j < languages2.length; j++) {
						System.out.println("COPY "
								+ languages2[j].getName()
								+ " TO "
								+ new File(dictionaryDest, languages[i]
										.getName()).getAbsolutePath());
						IOUtils.copy(languages2[j], new File(dictionaryDest,
								languages[i].getName()));
					}
				} else {
			 ...
			 */
			File templatesPwsFolder = new File(webapproot, "app/templates/pws/"
					+ newName);
			File templatesPwsWEBINFFolder = new File(templatesPwsFolder,
					"WEB-INF");
			File templatesPwsWEBINFLibFolder = new File(
					templatesPwsWEBINFFolder, "lib");
			...
			if (packagesExt.length() > 0) {
				...
				File packagesDir = new File(studioConfiguration.getCommonDir().getFile(),
						"packages");
				...
				if (packagesStartIndex != -1) {
					packagesStartIndex = packages.lastIndexOf(",",
							packagesStartIndex);
					int packagesEndIndex = packages.indexOf(endPackagesExt);
					if (packagesEndIndex != -1) {
						packagesEndIndex += endPackagesExt.length();
						packages = packages.substring(0, packagesStartIndex)
								+ packages.substring(packagesEndIndex);
					}
				}
			...
			File runtimexml = new File(extFolder, newName
					+ "-runtime-beans.xml");
			...
			File studioPartnerBeans = new File(webinf,
					"studio-partner-beans.xml");
			PwsInstall.insertEntryKey(studioPartnerBeans, runJars, designJars,
					newName);

			File templatesPwsRootFolder = new File(webapproot,
					"app/templates/pws/");
			File templatesPwsRootWEBINFFolder = new File(
					templatesPwsRootFolder, "WEB-INF");
			File projectSpringApp = new File(templatesPwsRootWEBINFFolder,
					"project-springapp.xml");
			PwsInstall.insertImport(projectSpringApp, newName
					+ "-runtime-beans.xml");

			File projectPartnerBeans = new File(templatesPwsRootWEBINFFolder,
					"project-partner-beans.xml");
			PwsInstall.insertEntryKey(projectPartnerBeans, runJars, designJars,
					newName);
	...
	public FileUploadResponse uploadJar(MultipartFile file) {
		...
		try {
			...
			if (dotindex == -1)
				...
				throw new IOException("Please upload a jar file, not a " + ext
						+ " file");
			}

			File destDir = new File(WMAppContext.getInstance()
					.getAppContextRoot());
