...
public class CopyRuntimeJarsTask extends Task {
    ...
    private File getRuntimeJarFile() {
        ...
        File runtimeJarFile = new File(this.from, runtimeJarName);
        ...
        runtimeJarFile = new File(this.from, DEFAULT_RUNTIME_JAR_NAME);
        if (runtimeJarFile.exists()) {
            return runtimeJarFile;
        }
    ...
    protected FileSet getProjectJarFileSet() {
        ...
        try {
            projectJarSet.setDir(((LocalFile) this.wmProject.getRootFolder()).getOriginalResource());

            projectJarSet.createInclude().setName(ProjectConstants.LIB_DIR + "/**/*.jar");

            File includeList = new File(((LocalFile) this.wmProject.getWebAppRootFolder()).getOriginalResource(), "resources/.includeJars");
    ...
    protected List<String> copyModules() throws IOException {
        ...
        for (File file : mm) {
            if (file.isDirectory()) {
                ...
                copyDirs.setTodir(new File(((LocalFile) this.wmProject.getClassOutputFolder()).getOriginalResource(), file.getName()));
