 ...
 */
public class JavaServiceDefinitionTest extends WMTestCase {
    ...
    public void setUp() throws Exception {
        ...
        File classFile = new ClassPathResource(this.CLASS_NAME.replace('.', '/') + ".class").getFile();
        ...
        this.def = new JavaServiceDefinition(this.CLASS_NAME, "serviceId", classRoots, libDirs, exclude);
    }
    ...
    public void testOperation1() {
        ...
        assertEquals(String.class.getName(), oper.getParameterTypes().get(0).getTypeDefinition().getTypeName());
        assertEquals(int.class.getName(), oper.getParameterTypes().get(1).getTypeDefinition().getTypeName());
    ...
    public void testOperation2() {
        ...
        assertEquals(String.class.getName(), so.getParameterTypes().get(0).getTypeDefinition().getTypeName());
    }
    ...
    public void testOperation3() {
        ...
        assertEquals(JavaServiceDefinitionClass.class.getName(), so.getReturnType().getTypeDefinition().getTypeName());
    ...
    public void testGetTypes() {
        List<TypeDefinition> types = this.def.getLocalTypes();
        assertEquals("types: " + types, 1, types.size());
        for (TypeDefinition type : types) {
            if (type.getTypeName().equals(this.CLASS_NAME)) {
                // good
            } else {
    ...
    public void testRuntimeService() {
        assertFalse(this.def.isLiveDataService());
    }

    private ServiceOperation getOperation(String name) {

        for (ServiceOperation so : this.def.getServiceOperations()) {
