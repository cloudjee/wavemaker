...
dojo.declare("ResourceManager", wm.Page, {
    ...
    renameItem: function() {
        ...
        app.prompt(this.getDictionaryItem("PROMPT_NAME", {
            oldName: this.selectedItem.getItemName()
        }), this.selectedItem.getItemName(), dojo.hitch(this.selectedItem, "setItemName"));
        /*
    ...
    fileUploadCompleted: function(inSender, fileList) {
        ...
        for (var i = 0; i < parent.treeNode.kids.length; i++) {
            if (parent.treeNode.kids[i].content == filename) replaceIndex = i;
        }

        if (replaceIndex == -1) {
            newItem = addResourceBinderNodes(parent.treeNode, {
                file: filename,
                files: [],
                type: "file"
            }, false);
        ...
        if (newItem.finishFileUpload) newItem.finishFileUpload();
    },

    start: function() {
        this.uploadButton._serviceVariable._operationInfo = {
            parameters: wm.typeManager.getType("AnyData").fields
        };
    ...
    dragDropItem: function() {
        ...
        var _this = this;
        studio.resourceManagerService.requestAsync("renameFile", [moveItem.getFilePath(), targetItem.getFilePath() + "/" + moveItem.getItemName(), false], function(response) {
            try {
                if (!response) {
    ...
    loadResourcesData: function(keepOpenFolders) {
        ...
        var tree = this.tree;
        studio.resourceManagerService.requestAsync("getFolder", [rootPath || ""], function(rootfolder) {
            var openFolderHash;
            if (keepOpenFolders && _this.resourcesFolder) {
                tree.openFolderHash = {
                    isOpen: true,
                    children: _this.resourcesFolder.buildOpenFolderStateHash()
                };
            } else {
                tree.openFolderHash = {};
            }
    ...
    isBlocked: function(inItem) {
        ...
        for (var i = 0; i < this.blockedFolders.length; i++) {
            if (path.indexOf(this.blockedFolders[i]) == 0) return true;
        }
        return false;
    },
...
dojo.declare("wm.ResourceMover", wm.DragDropper, {
    ...
    updateAvatar: function() {
        ...
        if (this.target) {
            ...
            this.setAvatarContent(studio.resourcesPage.page.getDictionaryItem("AVATAR_DROP", {
                caption: this.info.caption,
                target: dn
            }));
        }
    },
...
wm.ResourceItem.extend({
    ...
    deleteItem: function() {
        ...
        app.confirm(studio.resourcesPage.page.getDictionaryItem("CONFIRM_DELETE", {
                type: this instanceof wm.FolderResourceItem ? "folder" : "file",
            ...
            dojo.hitch(this, function() {
                ...
                studio.resourceManagerService.requestAsync("deleteFile", [this.getFilePath()],
                    dojo.hitch(this, function(result) {
                        try {
                            if (!result) {
                                app.alert(studio.resourcesPage.page.getDictionaryItem("ALERT_DELETE_FAILED", {
                                    fileName: this.getItemName()
                                }));
                                return;
                            }
    ...
    setItemName: function(inName, nosave) {
        ...
        var _this = this;
        var manager = studio.resourcesPage.getComponent("resourceManager");
        studio.resourceManagerService.requestAsync("renameFile", [this.getFilePath(), newpath, false], function(result) {
            try {
                if (!result) {
                    app.alert(studio.resourcesPage.page.getDictionaryItem("ALERT_RENAME_FAILED"));
                    return;
                } else if (result != inName) app.alert(studio.resourcesPage.page.getDictionaryItem("ALERT_RENAME_SUCCESS", {
                    fileName: result
                }));
    ...
    unselectSelf: function() {
        if (this.treeNode.selected) this.treeNode.tree.deselect();
    },
...
wm.FolderResourceItem.extend({
    addNewFolder: function(inName) {
        ...
        var newFolder = new wm.FolderResourceItem({
            itemName: inName
        });
        newFolder.treeNode = new wm.ResourceTreeNode(this.treeNode, {
            file: {
                file: inName,
                files: [],
                type: "folder"
    ...
    addNewFile: function(inName) {
        ...
        newFile.treeNode = new wm.ResourceTreeNode(this.treeNode, {
            file: {
                file: inName,
                files: [],
                type: "file"
    ...
    buildOpenFolderStateHash: function() {
        ...
        node.forEachChild(function(childIn) {
            ...
            if (child instanceof wm.FolderResourceItem) {
                var tmpresult = {
                    children: (!child.treeNode.closed) ? child.buildOpenFolderStateHash() : {},
