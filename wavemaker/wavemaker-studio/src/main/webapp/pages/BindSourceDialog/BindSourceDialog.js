...
dojo.declare("BindSourceDialog", wm.Page, {
        ...
        if (inNode) {
            ...
            if (inNode.object) {
                ...
                if (nodeIsAppLevel != targetIsAppLevel) {
                    if (nodeIsAppLevel) ownerString == "app.";
                    else ownerString = "[" + wm.decapitalize(studio.project.pageName) + "].";
    ...
    applyButtonClick: function() {
        if (this.binderSource.applyBinding(this.targetProps)) { /* If the cancel button is NOT showing, then don't autoclose the dialog after applying a binding */
