...
dojo.declare("ImportThirdPartyAPI", wm.Page, {
    ...
    onError: function(inSender, inError) {
        if (inError == "Please upload a zip file") app.alert(this.getDictionaryItem("UPLOAD_A_ZIP"));
        else if (inError.match(/Please upload a zip file, not a (.*) file/)) {
            var result = inError.match(/Please upload a zip file, not a (.*) file/);
            app.alert(this.getDictionaryItem("UPLOAD_A_ZIP_NOT_A", {
                fileType: result[1]
            }));
        } else app.alert(this.getDictionaryItem("ONERROR"));
    },
    done: function() {
        this.close();
        ...
        var def = dojo.rawXhrPost({
            url: "/ConfigurationTool/InstallService.download",
        ...
        def.addCallback(function() {
            ...
            studio.setUserSettings({
                reopenProject: studio.project.projectName
            });
