...
dojo.declare("wm.DesignHandles", wm.DomBorder, {
    ...
    showHide: function(inShow, inParent) {
        ...
        if (inShow != same || wasShowing != inShow) {
            for (var i = 0, h; h = this.handles[i]; i++) {
                try {
                    if (inShow) {
                        if (h.parentNode != parentNode) parentNode.appendChild(h);
                    } else {
                        if (h.parentNode == parentNode) parentNode.removeChild(h);
                    }
                } catch (e) {
...
dojo.declare("wm.DesignWrapper", wm.Designable, {
    ...
    setShowing: function(inShowing) {
        var showHandles = inShowing && studio.selected && dojo.indexOf(studio.selected, this.control) != -1;
        if (inShowing != this.showing && showHandles)
