...
dojo.declare("wm.JsonRpcService", wm.Service, {
	...
	invoke: function(inMethod, inArgs, owner, invoker, inLoop, inLongDeferred) {
	   ...
	   if (responseTime && responseTime == "long") {
			...
			requestId = dd.results[0].result;
			this._service.sync = savedSync;
			inArgs.push("longResponseTime");
			inArgs.push(requestId);
		} else if (inLoop && inMethod == "getResponseFromService") {
			requestId = inArgs[0];
		}
		...
		// longPollingResponse gets the deferred generated by the service call, and we return the deferred that is NOT triggered by the service call;
		// the caller of invoke getse back the longDeferred which is only executed when onLongPollingResponse says its time to invoke it.
		d.addBoth(dojo.hitch(this, "onLongPollingResponse",owner, invoker, inLoop, responseTime, requestId, longDeferred)); 
		d = longDeferred; // so that we return the longDeferred to the caller
	    } else {
