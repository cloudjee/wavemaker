 ...
 dojo.declare("wm.PropertyInspector", wm.AccordionLayers, {
     ...
     */
     ...
     isEditableProp: function(inProp, allowStyleInspector, skipIsAdvanced) {
         if (!skipIsAdvanced && (inProp.advanced && !this.isAdvancedMode() || !inProp.requiredGroup && this.isRequiredMode())) {
             return false;
         }

         if (inProp.group == "style" && inProp.editor != "wm.prop.StyleEditor" && !allowStyleInspector) return false; // handled by the style inspector only
         if (inProp.ignore) return false;
         if (inProp.method) return false;
         if (inProp.bindTarget || inProp.bindable) return true; // shows up even if writeonly/hidden are true; user can't set the value but can bind the value
         ...
         if (inGroup) {
             ...
             dojo.forEach(inGroup.subgroups, function(subgroup) {
                 if (subgroup.props.length && (subgroup.props.length > 1 || subgroup.props[0].editor != "wm.prop.FieldGroupEditor") && !subgroup.noDisplayName && (subgroup.name != "widgetName" || count > 0)) {
                     count++;
                     this.addSubGroupIndicator( /*(inGroup.displayName || inGroup.name) + " - " + */ (subgroup.displayName || subgroup.name), inLayer, this.isAdvancedMode() || dojo.some(subgroup.props, function(prop) {
     ...
     */
     generateButton: function(inComponent, inProp, inLayer) {
         ...
         var b = new wm.Button({
             ...
             width: "150px",
             //this.captionSize.match(/\%/) ? (100 - parseInt(this.captionSize)) + "%" : "100%",
             //height: "30px",
             caption: (inProp.shortname || inProp.name),
             _classes: {
                 domNode: ["StudioButton", "wmPropertyButton", "wmPropertyButton" + inProp.name, inProp.isPublished ? "isPublishedProp" : ""]
             },
             margin: "4,2,4,2",
             propDef: inProp,
     ...
     generatePanelForEditor: function(inParent, inName) {
         return new wm.Panel({
             ...
             width: "100%",
             height: "30px",
             /* This height is ignored; we call setBestHeight after we're done */
     ...
     getFormattedBoundValue: function(inType, inSource, inExpr) {
         ...
         inValue = inSource;
         else if (inExpr === undefined || inExpr === null || inExpr === "" || String(inExpr).match(/^\s*$/)) inValue = "";
         else if (inExpr == "true") inValue = "bool: true";
         else if (inExpr == "false") inValue = "bool: false";
         else if (inExpr == "null") inValue = "null: null";
         else if (inExpr == "null") inValue = "null: null";
         else if (String(inExpr).match(/\d/) && !isNaN(inExpr)) { // isNaN("") == isNaN(" ") == false
             if (inType.toLowerCase() == "data" || inType.toLowerCase() == "time" || inType.toLowerCase() == "java.util.date") {
                 inValue = "date: " + dojo.date.locale.format(new Date(Number(inExpr)), {
                     selector: "date"
                 });
             } else {
                 inValue = "numb: " + inExpr;
             }
         } else if (inExpr == "(binding data)") inValue = inExpr;
         else {
             var matches = inExpr.match(/^\s*\"([^\"]*)\"\s*$/);
             if (matches) inValue = "\"" + matches[1] + "\"";
             else inValue = "expr: " + inExpr;
     ...
     createBindEditor: function(inProp, editorProps, e, isBound, inComponent, optionalAppendToHashName) {
         ...
         var bindEditorProps = dojo.mixin(editorProps, {
             captionSize: (e.captionSize == "100%") ? "80px" : e.captionSize,
             // 100% for checkbox; need more than 16px to show bind value
             disabled: true,
             _classes: {
                 domNode: ["wminspector-boundvalue", "StudioEditor", inProp.isPublished ? "isPublishedProp" : ""]
             },
             hint: inProp.ignoretmp && inProp.ignoreHint ? this.ignoreHintPrefix + inProp.ignoreHint : "",
             // a bound editor of an ignored property
             resetButton: true,
             showing: isBound,
