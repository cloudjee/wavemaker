...
Studio.extend({
    addPartnerServices: function() {
        if (!this._serviceMenuStructure) {
            ...
            for (var i = 0; i < menus.length; i++) {
                if (menus[i].idInPage == "servicesPopupBtn") this._serviceMenuStructure = menus[i];
                else if (menus[i].idInPage == "insertPopupBtn") this._insertMenuStructure = menus[i];
            }
        }
    ...
    addComponentMenuItem: function(inTab, inName, inDescription, inImage, inClass, inProps) {
        ...
        try {
            ...
            if (inProps.partnerService) {
                if (!this._partnerServicesStructure) this.addPartnerServices();
      ...
      var newId = inName + "MenuItem";
        ...
        if (inProps) delete inProps.parentMenu;
    },
    ...
    disableMenuBar: function(inDisabled) {
        try {
            ...
            this.servicesPopupBtn.set("disabled", inDisabled || this.allowDisablingOfServiceItems && this.isCloud() && this._runRequested);
        ...
        */
            ...
            if (inDisabled) if (studio.isCloud()) {
