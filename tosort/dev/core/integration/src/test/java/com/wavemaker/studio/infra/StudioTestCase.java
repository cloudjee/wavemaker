 ...
 * 
 */
@ContextConfiguration(locations = { "webapp:/WEB-INF/studio-services.xml", "webapp:/WEB-INF/studio-managers.xml",
    "webapp:/WEB-INF/studio-security.xml", "webapp:/WEB-INF/studio-controllers.xml" }, loader = StudioTestCaseContextSupport.class)
public abstract class StudioTestCase extends SpringTestCase {
    ...
    public void setUp() throws Exception {
        ...
        try {
            this.wmHome = IOUtils.createTempDirectory("testDirFor_" + getClass().getName(), ".tmp/");
        } catch (IOException e) {
            throw new WMRuntimeException(e);
        }

        LocalStudioConfiguration config = (LocalStudioConfiguration) getBean("studioConfiguration");
        config.setTestWaveMakerHome(this.wmHome);
        assertTrue(config.getWaveMakerHome().exists());
        Assert.assertEquals(this.wmHome.getPath(), config.getWaveMakerHome().getFile().getPath());
    ...
    public void tearDown() throws Exception {
        if (null != this.wmHome && this.wmHome.exists()) {
            IOUtils.deleteRecursive(this.wmHome);
        }
    }
    ...
    protected void populateProject(File projectRoot, boolean addWebXml) {
        ...
        try {
            ...
            FileUtils.writeStringToFile(new File(webapproot, "login.html"), INDEX_HTML_TEXT);
            ...
            File serviceFile = new File(projectRoot, "services/aservice/src/foo.properties");
    ...
    protected File makeProject(String projectName, boolean noTemplate) throws Exception {

        File expectedPath = new File(new File(getTestWaveMakerHome(), LocalStudioConfiguration.PROJECTS_DIR), projectName);
        assertFalse(expectedPath.exists());

        String op = noTemplate ? "newProjectNoTemplate" : "newProject";

        Object o = invokeService_toObject("studioService", op, new Object[] { projectName });

        assertTrue("failed to create project: " + expectedPath, expectedPath.exists());
